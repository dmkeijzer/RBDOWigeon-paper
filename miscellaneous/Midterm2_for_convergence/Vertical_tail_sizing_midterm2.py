...
class VT_sizing:
    ...
    def Sweep(self, AR, Sweepm, n, m):
        ...
        tanSweep_n = tanSweep_m - 4 / (AR * 4) * (n - m) / 100 * (1 - self.taper) / (
            1 + self.taper
        )
    ...
    def VT_stability(self, lv):
        ...
        Sv = (
            ...
            * (
                ...
                - Cnb_w_rear * self.Srear * self.brear / (self.S * bmax)
            )
            / (-CYb_v)
            * bmax
            / lv
    ...
    def final_VT_rudder(self, nE, Tt0, yE, lv, br_bv, cr_cv):
        ...
        if isinstance(br_bv, float) and isinstance(self.ARv, float):
            Sv = max(
                self.VT_controllability(nE, Tt0, yE, lv, br_bv, cr_cv),
                self.VT_stability(lv),
            )
