...
class componentdrag:
    def __init__(
        ...
        self.AR = (
            AR
            + s1 * winglet_dAR(AR * 2, self.h_wl1, np.sqrt(AR * S_ref))
            + s2 * winglet_dAR(AR * 2, self.h_wl2, np.sqrt(AR * S_ref))
        )
        self.e = e
        self.M = M_cr
        self.k = k
    ...
    def Swet_f(self):
        ...
        return (np.pi * self.d / 4) * (
            (
                (1 / (3 * self.l1**2))
                * ((4 * self.l1**2 + ((self.d**2) / 4)) ** 1.5 - ((self.d**3) / 8))
            )
            - self.d
            + 4 * self.l2
            + 2 * np.sqrt(self.l3**2 + (self.d**2) / 4)
        )
    ...
    def Re_f(self):
        ...
        return min(
            (self.rho * self.V * (self.l) / self.mu), 38.21 * (self.l / self.k) ** 1.053
        )

    def Re_w(self):

        return min(
            (self.rho * self.V * (self.c) / self.mu), 38.21 * (self.c / self.k) ** 1.053
        )
    ...
    def Cf_f(self):
        ...
        Cfturb = 0.455 / (
            ((np.log10(self.Re_f())) ** 2.58) * (1 + 0.144 * self.M * self.M) ** 0.65
        )
    ...
    def Cf_w(self):
        ...
        Cfturb = 0.455 / (
            ((np.log10(self.Re_w())) ** 2.58) * (1 + 0.144 * self.M * self.M) ** 0.65
        )
    ...
    def FF_w(self):
        return (1 + 0.6 * self.toc / (self.xcm) + 100 * self.toc**4) * (
            1.34 * (self.M**0.18) * (np.cos(self.sweepm)) ** 0.28
        )

    def CD0(self):

        self.CD0_f = (1 / self.S_ref) * (
            self.Cf_f() * self.FF_f() * self.IF_f * self.Swet_f()
        )
        self.CD0_v = (1 / self.S_ref) * (
            self.Cf_w() * self.FF_w() * self.IF_v * self.Swet_v()
        )
