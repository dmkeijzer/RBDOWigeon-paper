...
file_path = r"C:\Users\damie\OneDrive\Desktop\Damien\Wigeon_proj\logs\valid_data\Baseline\Deterministic_Jun__1_00.15_hist.csv"
...
def plot_design_params(save_bool):
    ...
    # Control margin
    # y6 = data_csv["ctrl_margin"][conv_lst].to_numpy()
    ...
    axs[1, 1].plot(y9/3.6e6,label=r'Mission Energy')
    axs[1, 1].set_xlabel(r"$n_{th}$ Converged design")
    axs[1, 1].set_ylabel(r'Energy [kWh]')
    axs[1, 1].grid()
    # axs[1, 1].legend()

    fig.tight_layout()
    if save_bool:
        plt.savefig(os.path.join(download_path, os.path.split(file_path)[-1][:-4]) + ".pdf", bbox_inches= "tight")
    else:
        plt.show()
