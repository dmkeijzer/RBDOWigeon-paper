...
if __name__ == "__main__":  # Required due to the multiprocessing involved
    # USER INPUT FROM configuration file
    # ==========================================================================================================
    ...
    Ecruise_rv, Eclimb_rv, Edesc_rv, Eloit_cr_rv, Eloit_hov_rv = mcs.get_energy_distr(
        mission_res
    )
    ...
    np.save(
        os.path.join(dir_path, "mcs_metric", r"mcs_metric_samples" + label + ".npy"),
        sample_hist,
    )
    np.save(
        os.path.join(dir_path, "mcs_metric", r"mcs_metric_results" + label + ".npy"),
        mission_res,
    )
    ...
    for name, obj in zip(str_lst, obj_lst):
        with open(
            os.path.join(dir_path, "mcs_metric", name + label + ".pkl"), "wb"
        ) as f:
            pickle.dump(obj, f)
