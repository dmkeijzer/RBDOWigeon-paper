...
import modules.integration.integration_class_baseline as int_class
import input.constants_final as const

label = ("_".join(time.asctime().split(" ")[1:-1])).replace(":", ".")[:-3]
path_directory = os.path.join(
    os.path.join(str(list(pl.Path(__file__).parents)[1]), "output", "baseline"),
    "run_" + label,
)
...
class design_optimization(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        optim_outputs, internal_inputs, other_outputs, conv_hist = (
            optimisation_class.multirun(N_iter, optim_inputs=[])
        )

        if os.path.exists(
            os.path.join(path_directory, "Deterministic" + "_" + label + "_hist.csv")
        ):
            pd.DataFrame(conv_hist).to_csv(
                os.path.join(
                    path_directory, "Deterministic" + "_" + label + "_hist.csv"
                ),
                mode="a",
                header=False,
                index=False,
            ...
            pd.DataFrame(conv_hist).to_csv(
                ...
                header=other_outputs["lines"][:, 0].flatten(),
                index=False,
            )
