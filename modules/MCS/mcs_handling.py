...
def sample_mission_data(chunksize):
    ...
    dist_samples = (
        np.array(stat.genextreme.rvs(0.94, loc=309.40, scale=84.96, size=chunksize))
        * 1000
    )
    ...
    while np.size(dist_samples[dist_samples < const.minimum_dist]) != 0:
        ...
        dist_samples[dist_samples < const.minimum_dist] = (
            stat.genextreme.rvs(0.94, loc=309.40, scale=84.96, size=n_resample) * 1000
        )
...
def get_mcs_results(MissionClass, conv_target=0.85, chunksize=100, test=False):
    ...
    while conv_condition:
        ...
        with mp.Pool(os.cpu_count()) as p:
            mission_res_chunk = p.starmap(
                MissionClass.single_sample_monte_carlo, sim_samples
            )

        mission_res.append(
            mission_res_chunk
        )  # array [[x00, x01, x02, x03, x04, array[y00, y01, y02, y03, y04],
        #                                                                     x11, x11, x12, x13, x14, array[y10, y11, y12, y13, y14]] etc
        conv_metric_lst.append(
            np.std(np.vstack(np.array(mission_res, dtype=object))[:, 0])
        )
        i += 1
        ...
        try:
            print(
                f"Delta Q = {np.absolute(conv_metric_lst[-2] - conv_metric_lst[-1])} "
            )
            if (
                np.absolute(
                    (conv_metric_lst[-2] - conv_metric_lst[-1])
                    / conv_metric_lst[-2]
                    * 100
                )
                < conv_target
                and np.absolute(
                    (conv_metric_lst[-2] - conv_metric_lst[-3])
                    / conv_metric_lst[-3]
                    * 100
                )
                < conv_target
...
def get_energy_distr(mission_res):
    ...
    with mp.Pool(os.cpu_count()) as p:
        Ecruise_rv, Eclimb_rv, Edesc_rv, Eloit_cr_rv, Eloit_hov_rv = p.map(
            RandVar, data
        )
