...
if Prop_config == 1 or Prop_config == 2:
    ...
    print(
        "Equivalent radius for one propeller (from Disk Loading):",
        np.sqrt(disk.A / np.pi),
        "[m]",
    )
    ...
    print("--- Energy ---")
    print(
        "Energy for hover (assuming 4 minutes in total for a flight):",
        P_h.P_hover() * 1.2 * (4 / 60) / 1000,
        "[kWh]",
    )
    time_cruise = 300 * 1000 / V_cruise
    print(
        "Energy for cruise (assuming 300 km of flight at V_Cruise):",
        P_cr.P_cr() * 1.2 * (time_cruise / 3600) / 1000,
        "[kWh]",
    )
    req_energy = (
        P_h.P_hover() * 1.2 * (4 / 60) / 1000
...
if Prop_config == 3:
    ...
    r_out_wing_eng = (
        (xc_wing_eng_end - xc_wing_eng_start) * c_r / 2
        - (xc_wing_eng_end - xc_wing_eng_start)
        * (1 - taper)
        * c_r
        * xb_wing_eng_start
        / 2
    ...
    area_wing_prop = np.pi * (
        r_out_wing_eng**2 - (wing_prop_hub_ratio * r_out_wing_eng) ** 2
    )
    ...
    print(
        "Max thrust of each wing engine:",
        MTOW * (1 - area_ratio_tilt) * TW_ratio / 2,
        "[N]",
    )
    print(
        "Max thrust of each tilt engine:", MTOW * area_ratio_tilt * TW_ratio / 4, "[N]"
    )

    P_cr_tilt = prop.PropulsionCruise(
        mass_tilt_eng,
        N_cruise,
        area_tilt_eng,
        eff_P_cr,
    ...
    print("--- P for max thrust ---")
    P_max_tilt = prop.PropulsionHover(
        TW_ratio * mass_tilt_eng,
        4,
        area_tilt_eng,
        eff_D_h,
        eff_F_h,
    ...
    P_max_wing = prop.PropulsionHover(
        TW_ratio * mass_wing_eng,
        2,
        area_wing_prop,
        eff_D_h,
        eff_F_h,
    ...
    print(
        "Energy for hover (assuming 4 minutes in total for a flight):",
        P_hover * 1.2 * (4 / 60) / 1000,
        "[kWh]",
    )
    time_cruise = 300 * 1000 / V_cruise
    print(
        "Energy for cruise (assuming 300 km of flight at V_Cruise):",
        P_cr_tilt.P_cr() * 1.2 * (time_cruise / 3600) / 1000,
        "[kWh]",
    )
    req_energy = (
        P_hover * 1.2 * (4 / 60) / 1000
...
if Prop_config == 1:
    ...
    print(
        "With leading edge distributed propulsion, this area can be reduced to",
        LE_prop.S1(),
        "[m^2]",
    )
...
print(
    "The propulsion goes from",
    xb_prop_start,
    "to",
    xb_prop_end,
    "of the half wing, which has a span of",
    b / 2,
    "[m]",
)
...
if Prop_config == 2:
    ...
    print(
        "The height of the (fully laminar) BL at the engine closest to the fuselage is",
        BL_height_lam_inb,
        "[m], which corresponds to BL/D ratio of",
        BL_height_lam_inb / (2 * r_out),
        "for a fan diameter of",
        2 * r_out,
        "[m]",
    )
    print(
        "The height of the (fully laminar) BL at the engine furthest from the fuselage is",
        BL_height_lam_outb,
        "[m], which corresponds to BL/D ratio of",
        BL_height_lam_outb / (2 * r_out),
        "for a fan diameter of",
        2 * r_out,
        "[m]",
    )
    print(
        "The height of the (fully turbulent) BL at the engine closest to the fuselage is",
        BL_height_tur_inb,
        "[m], which corresponds to BL/D ratio of",
        BL_height_tur_inb / (2 * r_out),
        "for a fan diameter of",
        2 * r_out,
        "[m]",
    )
    print(
        "The height of the (fully turbulent) BL at the engine furthest from the fuselage is",
        BL_height_tur_outb,
        "[m], which corresponds to BL/D ratio of",
        BL_height_tur_outb / (2 * r_out),
        "for a fan diameter of",
        2 * r_out,
        "[m]",
    )
