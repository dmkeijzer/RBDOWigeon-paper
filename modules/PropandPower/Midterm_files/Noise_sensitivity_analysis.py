...
if Prop_config == 1:
    ...
    P_br_cruise = (
        P_cr_estim / N_cruise * 1 / 1000 * eff_B_cr * eff_PE_cr * eff_M_cr / 1.2
    )
    ...
    print(
        "Noise level in cruise, total:",
        brr.sum_noise(np.ones((1, N_cruise))[0] * noise.SPL_cr()),
        "[dB]",
    )
    print("")
    ...
    print(
        "Noise level in hover, total:",
        brr.sum_noise(np.ones((1, N_hover))[0] * noise.SPL_hover()),
        "[dB]",
    )
    print("")
...
if Prop_config == 2:
    ...
    print(
        "For config 2, the maximum allowable rpm in hover are:", rpm_max_hover, "[rpm]"
    )
    ...
    print(
        "For config 3, the maximum allowable rpm for the tilting engines are:",
        rpm_max_tilt,
        "[rpm]",
    )
    print(
        "For config 3, the maximum allowable rpm for the wing engines are:",
        rpm_max_wing,
        "[rpm]",
    )
    print("")
    ...
    print(
        "Noise level in cruise, total:",
        brr.sum_noise(np.ones((1, N_cruise))[0] * noise_tilt.SPL_cr()),
        "[dB]",
    )
    print("")
    ...
    print(
        ...
        brr.sum_noise(
            [
                noise_tilt.SPL_hover(),
                noise_tilt.SPL_hover(),
                noise_tilt.SPL_hover(),
                noise_tilt.SPL_hover(),
                noise_wing.SPL_hover(),
                noise_wing.SPL_hover(),
            ]
        ),
        "[dB]",
