...
class PropulsionHover:
    ...
    def P_hover(self):
        ...
        if self.ducted == 1:
            return (
                0.5 * self.T_h ** (3 / 2) / np.sqrt(self.rho * self.n * self.A)
            ) / self.eff_hover
        if self.ducted == 0:
            return (
                self.T_h ** (3 / 2) / np.sqrt(2 * self.rho * self.n * self.A)
            ) / self.eff_hover
        else:
...
class ActuatorDisk:
    def __init__(self, D_inner_ratio, D_prop_pure_hover_ratio):
        ...
        atm_flight = ISA(
            self.h_cruise
        )  # atmospheric conditions during flight   # Idk if this actually works
        atm_LTO = ISA(
            0
    ...
    def P_ideal(self):
        return (
            ...
            * self.S
            * self.CD
            * (np.sqrt(self.CD * self.S / self.A_hover() + 1) + 1)
        )
