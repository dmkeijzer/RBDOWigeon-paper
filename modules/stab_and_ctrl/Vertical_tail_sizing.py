...
class VT_sizing:
    def __init__(
        ...
        self.CLfwd, self.CLrear = (
            CLfwd,
            CLrear,
        )  # DESIGN FOR CRUISE Lift coefficients [-]
    ...
    def Sweep(self, AR, Sweepm, n, m):
        ...
        tanSweep_n = tanSweep_m - 4 / (2 * AR) * (n - m) / 100 * (1 - self.taper) / (
            1 + self.taper
        )
    ...
    def xacv(self, ARv, sweepTE):
        ...
        """
        ylemac = (
            self.initial_VT()[2] * 2 / 6 * (1 + 2 * self.taper_v) / (1 + self.taper_v)
        )
        ...
        xacv = (
            ...
            - self.initial_VT()[5]
            + xlemac
            + 0.25 * 0.5 * (self.initial_VT()[5] + self.initial_VT()[6])
        )
        # print(ylemac, xlemac)
        # print(xacv)
        return xacv
    ...
    def VT_controllability(self, r1, r2, nf, nE, Tt0, br_bv, cr_cv, ARv, sweepTE):
        ...
        Vv_V = 1
        Sv = N_total / (
            0.5 * self.rho * self.Vmc**2 * CLa_v * lv * Vv_V**2 * tau_r * br_bv * dr_max
        )
        # print("Ctrl: lv = ",lv)
    ...
    def VT_stability(self, ARv, sweepTE):
        ...
        Cnb_w_fwd = self.CLfwd**2 * (
            1 / (4 * np.pi * self.Afwd)
            - (
                np.tan(self.Sweepc4fwd)
                / (np.pi * self.Afwd + 4 * np.cos(self.Sweepc4fwd))
            )
            * (
                np.cos(self.Sweepc4fwd)
                - self.Afwd / 2
                - self.Afwd**2 / (8 * np.cos(self.Sweepc4fwd))
                - 6
                * (self.xacfwd - self.xcg)
                * np.sin(self.Sweepc4fwd)
                / (self.Afwd * self.c)
            )
        )
        Cnb_w_rear = self.CLrear**2 * (
            1 / (4 * np.pi * self.Arear)
            - (
                np.tan(self.Sweepc4rear)
                / (np.pi * self.Arear + 4 * np.cos(self.Sweepc4rear))
            )
            * (
                np.cos(self.Sweepc4rear)
                - self.Afwd / 2
                - self.Arear**2 / (8 * np.cos(self.Sweepc4rear))
                - 6
                * (self.xacrear - self.xcg)
                * np.sin(self.Sweepc4rear)
                / (self.Arear * self.c)
            )
        )
        ...
        Sv = (
            ...
            * (
                Cnb
                - Cnb_fus
                - Cnb_w_fwd * self.Sfwd * self.bfwd / (self.S * self.b)
                - Cnb_w_rear * self.Srear * self.brear / (self.S * self.b)
            )
            / (-CYb_v)
            * self.b
            / lv
    ...
    def final_VT_rudder(self, r1, r2, nf, nE, Tt0, br_bv, cr_cv, ARv, sweepTE):
        ...
        """
        if (
            isinstance(br_bv, (float, int))
            and isinstance(self.ARv, (float, int))
            and isinstance(self.sweepTE, (float, int))
        ):
            Sv = max(
                self.VT_controllability(
                    r1, r2, nf, nE, Tt0, br_bv, cr_cv, ARv, sweepTE
                ),
                self.VT_stability(ARv, sweepTE),
            )
            ...
            isinstance(br_bv, (float, int))
            and not isinstance(ARv, (float, int))
            and not isinstance(sweepTE, (float, int))
        ):
            Sv_stab = self.VT_stability(ARv, sweepTE)
            Sv_ctrl = self.VT_controllability(
                r1, r2, nf, nE, Tt0, br_bv, cr_cv, ARv, sweepTE
            )
        ...
        c_r = cr_cv * C_v
        c_r_root = (
            3 / 2 * c_r * (1 + self.taper_v) / (1 + self.taper_v + self.taper_v**2)
        )
    ...
    def plotting(self, r1, r2, nf, nE, Tt0, br_bv, cr_cv, ARv, sweepTE):
        if (
            ...
            not isinstance(self.ARv, (float, int))
            and isinstance(br_bv, (float, int))
            and isinstance(self.sweepTE, (float, int))
        ):
            Svstab = self.VT_stability(ARv, sweepTE)
            Svcontrol = self.VT_controllability(
                r1, r2, nf, nE, Tt0, br_bv, cr_cv, ARv, sweepTE
            )
            bv = self.final_VT_rudder(
                r1, r2, nf, nE, Tt0, br_bv, cr_cv, ARv=self.ARv, sweepTE=self.sweepTE
            ...
            isinstance(self.ARv, (float, int))
            and isinstance(br_bv, (float, int))
            and not isinstance(self.sweepTE, (float, int))
        ):
            Svstab = self.VT_stability(ARv, sweepTE)
            Svcontrol = self.VT_controllability(
                r1, r2, nf, nE, Tt0, br_bv, cr_cv, ARv, sweepTE
            ...
            plt.plot(
                self.sweepTE * 180 / np.pi,
                Svcontrol,
                label="Controllability for OEI condition",
            )
            ...
            plt.show()
            bv = self.final_VT_rudder(
                r1, r2, nf, nE, Tt0, br_bv, cr_cv, ARv=X, sweepTE=Y
            )[3]
            fig, ax = plt.subplots(1, 1)
            Sv_estimate = None
            # ax.add_artist(ab)
            ...
            Svstab = ax.contour(
                X, Y, Z, [self.VT_stability(ARv, sweepTE)], colors=["k"]
            )
            ...
            Svstab = ax.contour(
                ...
                Y,
                Z,
                [np.sqrt(self.VT_stability(ARv, sweepTE) * self.ARv)],
                colors=["k"],
            )
