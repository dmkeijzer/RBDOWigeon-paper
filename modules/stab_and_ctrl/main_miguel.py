    ...
    value = (
        ...
        * A
        / (
            2
            + np.sqrt(
                4 + ((A * beta / eta) ** 2) * (1 + (np.tan(Lambda_half) / beta) ** 2)
            )
        )
    )
    return value
...
if isinstance(ARv, (float, int)) and isinstance(sweepTE, (float, int)):
    vt_sizing.plotting(
        r1, r2, nf, nE, Tt0, br_bv=brbv, cr_cv=crcv, ARv=ARv, sweepTE=sweepTE
    )
    vt_sizing.plotting(
        r1, r2, nf, nE, Tt0, br_bv=1.0, cr_cv=0.24, ARv=ARv, sweepTE=sweepTE
    )
    Sv = vt_sizing.final_VT_rudder(
        r1, r2, nf, nE, Tt0, br_bv=1.0, cr_cv=0.24, ARv=ARv, sweepTE=sweepTE
    )[0]
    Svstab = vt_sizing.VT_stability(ARv, sweepTE)
    Svctrl = vt_sizing.VT_controllability(
        r1, r2, nf, nE, Tt0, br_bv=1.0, cr_cv=0.24, ARv=ARv, sweepTE=sweepTE
    )
    bv = vt_sizing.final_VT_rudder(
        r1, r2, nf, nE, Tt0, br_bv=1.0, cr_cv=0.24, ARv=ARv, sweepTE=sweepTE
    )[3]
    cvr = vt_sizing.final_VT_rudder(
        r1, r2, nf, nE, Tt0, br_bv=1.0, cr_cv=0.24, ARv=ARv, sweepTE=sweepTE
    )[1]
    cvt = vt_sizing.final_VT_rudder(
        r1, r2, nf, nE, Tt0, br_bv=1.0, cr_cv=0.24, ARv=ARv, sweepTE=sweepTE
    )[2]
    ...
    vt_sizing.plotting(
        r1, r2, nf, nE, Tt0, br_bv=0.8, cr_cv=0.35, ARv=ARv, sweepTE=sweepTE
    )

#### Plotting Elevator ####
elevator = Elevator_sizing(
    W,
    h,
    xcg,
    Zcg,
...
Ixx, Iyy, Izz, Ixz = weight.MMI(
    wmac=[cfwd, crear], toc=[0.17, 0.17], vpos_wing=[0.3, 1.7]
)
print("Iyy = %.5f , Ixx = %.5f, Izz = %.5f, Ixz = %.5f" % (Iyy, Ixx, Izz, Ixz))
print(
    "Kxx2 = %.5f, Kyy2 = %.5f, Kzz2 = %.5f, Kxz = %.5f"
    % (
        Ixx / (W / 9.80665 * b**2),
        Iyy / (W / 9.80665 * c**2),
        Izz / (W / 9.80665 * b**2),
...
if isinstance(ARv, float) and isinstance(sweepTE, float):
    ...
    print(
        "da-derivatives:",
        stability_derivatives.da_derivatives(Sa_S=0.115423, b1=47.0294, b2=99.0),
    )
    print(
        "dr-derivatives:", stability_derivatives.dr_derivatives(cr_cv=0.24, br_bv=1.0)
    )
    print("Kyy2 = %.5f" % (Iyy / (W / 9.80665 * c**2)))
    stability_derivatives.asym_stability_req(
        Ixx,
        Izz,
        Ixz,
        Sa_S=0.115423,
    ...
    C_X, C_Z, C_m, C_Y, C_l, C_n = stability_derivatives.return_stab_derivatives(
        Se_S=Se_S,
        be_b=be_b,
        Sa_S=0.115423,
        b1=47.0294,
