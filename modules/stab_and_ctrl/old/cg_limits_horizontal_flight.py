...
def values_conf_1_2(conf, sens, sens_value):
    datafile = open(
        os.path.join(root_path, "data/inputs_config_%.0f.json" % (conf)), "r"
    )
    data = json.load(datafile)
    datafile.close()
...
def cg_range_conf_1_2(conf, s, s_value, AR_t, AR, CL_t):
    ...
    o = CLafwd * xacfwd_stab + CLarear * (lfus - 0.75 * cfwd) * Srear / Sfwd * (
        1 - deda
    )
    ...
    oo = (
        ...
        / cfwd
        * (
            CLfwd * xacfwd_control
            + CLrear * xacrear * Srear / Sfwd
            - Cmacrear * Srear / Sfwd * crear
        )
        - Cmacfwd
    )
...
cg_CL = cg_range_conf_1_2(1, s=True, s_value=sense_value, AR_t=False, AR=AR, CL_t=True)[
    0
]
...
cg_CL = cg_range_conf_1_2(2, s=True, s_value=sense_value, AR_t=False, AR=AR, CL_t=True)[
    0
]
plt.plot(sense_value, cg_CL)
plt.xlabel(r"Increase in $C_{L_{fwd}}$ [%]", fontsize=14)
...
CLa = C_L_a(
    1,
    True,
    values_sens_1_2(1, True, True, sense_value, AR_t=False, AR=AR, CL_t=False)[8],
    0,
)
...
def CmqCzq(conf, sens, sens_value, AR_t, AR):
    ...
    if conf == 1 or conf == 2:
        ...
        Czq = (CLafwd * (xcg - xacfwd) / cfwd) - CLarear * Srear / Sfwd * (
            xacrear - xcg
        ) / cfwd
        Cmq = -(
            CLafwd * Sfwd / S * (xcg - xacfwd) ** 2 / cfwd**2
            + CLarear * Srear / S * (xacrear - xcg) ** 2 / cfwd**2
        )
    else:
        xcg = 1.7947
        values = values_conf_3(sens, sens_value, AR_t, AR)
    ...
    print(
        "Configuration %.0f: C_Z_q = %.3f 1/rad and C_m_q = %.3f 1/rad"
        % (conf, Czq, Cmq)
    )
    return Czq, Cmq
...
def Sr_Sfwd(conf, s, s_value, AR_t, AR, CL_t, Xcg, d):
    ...
    SrSfwd_control = (
        ...
        - CLfwd
        * (Xcg - xacfwd - d)
        / (CDrear * hfus / 2 - CLrear * (xacrear - Xcg) + Cmacrear * crear)
    )
...
for i in range(0, len(d)):
    SrSfwd = Sr_Sfwd(
        1, s=False, s_value=0, AR_t=False, AR=AR, CL_t=False, Xcg=Xcg, d=d[i]
    )
