...
class Wing_placement_sizing:
    def __init__(
        ...
        self.CLafwd = self.C_L_a(
            self.Afwd, self.Sweepc2fwd
        )  # Wing lift curve slopes for both wings [1/rad]
        self.CLarear = self.C_L_a(
            self.Arear, self.Sweepc2rear
        )  # Wing lift curve slopes for both wings [1/rad]
        ...
        self.Vr_V2 = Vr_V2
        self.Zcg = Zcg
        self.hover_calc = HoverControlCalcTandem(
            W / consts.g,
            n_rot_f,
            n_rot_r,
            self.xacfwd,
    ...
    def deps_da(self, Lambda_quarter_chord, b, lh, h_ht, A, CLaw):
        ...
        dsde_da = np.where(
            ...
            6.5
            * (self.rho * self.Pbr**2 * Sf**3 * self.CLdesfwd**3 / (lh**4 * self.W**3))
            ** (1 / 4)
            * (np.sin(phi * 6)) ** 2.5,
            0,
        )
    ...
    def Sweep(self, AR, Sweepm, n, m):
        ...
        tanSweep_n = tanSweep_m - 4 / (AR) * (n - m) / 100 * (1 - self.taper) / (
            1 + self.taper
        )
    ...
    def Sr_Sfwd(self, Xcg, elevator, d):
        ...
        c = (
            self.Sfwd / (self.Sfwd + self.Srear) * self.cfwd
            + self.Srear / (self.Srear + self.Sfwd) * self.crear
        )
        ...
        # print("de/da = ",deda)
        SrSfwd_stab = (
            -self.CLafwd * (Xcg - self.xacfwd) + CDafwd * (self.Zcg - self.dy)
        ) / (
            -self.CLarear * (1 - deda) * (self.xacrear - Xcg)
            + CDarear * (self.hfus - self.Zcg) * (1 - deda)
        )
        SrSfwd_control = (
            -self.Cmacfwd * self.cfwd
            + CDfwd * (self.Zcg - self.dy)
            - CLfwd
            * (Xcg - self.xacfwd)
            / (
                CDrear * (self.hfus - self.Zcg)
                - self.CLrear * (self.xacrear - Xcg)
                + self.Cmacrear * self.crear
            )
        )
    ...
    def plotting(self, x_min, x_max, dx, elevator, d, n_failures=2, y_cg=0):
        ...
        Sr = self.S - Sforward
        cforward = (
            self.cfwd * np.sqrt(Sforward / self.Afwd) / np.sqrt(self.Sfwd / self.Afwd)
        )
        ...
        o = (
            -self.Cmacfwd * cforward
            + CDfwd * (self.Zcg - self.dy)
            + self.CLfwd * elevator * xacfwd_1
            - self.Vr_V2
            * (
                CDrear * (self.hfus - self.Zcg) * Sr_Sfwd
                - self.CLrear * Sr_Sfwd * xacrear_1
                + self.Cmacrear * Sr_Sfwd * cr
            )
        )
        ...
        oo = (
            CDafwd * (self.Zcg - self.dy)
            + self.CLafwd * xacfwd_1
            - self.Vr_V2
            * (
                CDarear * (self.hfus - self.Zcg) * (1 - deda) * Sr_Sfwd
                - self.CLarear * Sr_Sfwd * xacrear_1 * (1 - deda)
            )
        )
