...
class Aircraft:
    def __init__(
        ...
        self.asym_sys = self.compute_asym_sys(
            V, self.Kxx2, self.Kzz2, self.Kxz, C_L, C_Y, C_l, C_n
        )
    ...
    def plot_results(self, V):
        ...
        u_motion, alpha_motion, theta_motion, pitch_motion = self.phugoid(
            T_p, delta_e, V, sys=self.sym_sys
        )
        ...
        u_motion, alpha_motion, theta_motion, pitch_motion = self.short_period(
            T_sp, delta_e, V, sys=self.sym_sys
        )
        ...
        beta_motion, phi_motion, p_motion, r_motion, extra_var = self.dutch_roll(
            T, delta_r, V
        )
        fig, axs = plt.subplots(2, 2, figsize=(7, 4), constrained_layout=True)
        ...
        beta_motion, phi_motion, p_motion, r_motion, extra_var = self.ap_roll(
            T, delta_a, V
        )
        ...
        beta_motion, phi_motion, p_motion, r_motion, extra_var = self.spiral(
            T, delta_a, V
        )
    ...
    def compute_sym_sys(self, V, Kyy2, C_X, C_Z, C_m):
        ...
        C_2 = np.array(
            [
                [C_x_u, C_x_a, C_z_0, C_x_q],
                [C_z_u, C_z_a, -C_x_0, (C_z_q + 2 * mu_c)],
                [0, 0, 0, 1],
                [C_m_u, C_m_a, 0, C_m_q],
            ]
        )
        C_3 = np.array([[C_x_d], [C_z_d], [0], [C_m_d]])
    ...
    def get_period_prop(self, Sys, V, sit):
        ...
        for i in ii:
            ...
            prop.append(
                [
                    "For \u03bb_%.0f,%.0f:  T_1/2 = %.4f [s]; P = %.4f [s]; damping = %.4f [-]"
                    % (i, i + 1, Ti, Pi, zetai)
                ]
            )
        return prop
    ...
    def get_period_prop_a(self, Sys, V, sit):
        ...
        for i in ii:
            ...
            prop.append(
                [
                    "For \u03bb_%.0f :  T_1/2 = %.3f [s]; P = %.3f [s]; damping = %.3f [-]"
                    % (i, Ti, Pi, zetai)
                ]
            )
        return prop
    ...
    def compute_asym_sys(self, V, Kxx2, Kzz2, Kxz, C_L, C_Y, C_l, C_n):
        ...
        C_1 = np.array(
            [
                [(C_y_b_dot - 2 * mu_b) * self.b / V, 0, 0, 0],
                [0, -0.5 * self.b / V, 0, 0],
                ...
                [
                    ...
                    0,
                    4 * mu_b * Kxz * self.b / V,
                    -4 * mu_b * Kzz2 * self.b / V,
                ],
            ]
        )
        C_2 = np.array(
            [
                [C_y_b, C_L, C_y_p, (C_y_r - 4 * mu_b)],
                [0, 0, 1, 0],
                [C_l_b, 0, C_l_p, C_l_r],
                [C_n_b, 0, C_n_p, C_n_r],
            ]
        )
        C_3 = np.array([[C_y_da, C_y_dr], [0, 0], [C_l_da, C_l_dr], [C_n_da, C_n_dr]])
        A_s = -np.dot(np.linalg.inv(C_1), C_2)
        B_s = -np.dot(np.linalg.inv(C_1), C_3)
        C_s = np.identity(4)
        D_s = np.array([[0, 0], [0, 0], [0, 0], [0, 0]])
        return matlab.ss(A_s, B_s, C_s, D_s)
    ...
    def dutch_roll(self, T, delta_r, V, delta_a=None):
        ...
        init = np.array(
            [beta_init, phi_init, p_init * self.b / (2 * V), r_init * self.b / (2 * V)]
        )
