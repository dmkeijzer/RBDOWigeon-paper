...
class Control_surface:
    ...
    def Clda(self, Sa_S, b1, b2, rear):
        ...
        Cldafwd = (
            ...
            * c_r_fwd
            / (self.Sfwd * self.bfwd)
            * (
                0.5 * (b_2fwd**2 - b_1fwd**2)
                + 2 * (self.taper - 1) / (3 * self.bfwd) * (b_2fwd**3 - b_1fwd**3)
            )
        )
        ...
        Cldarear = (
            ...
            * self.tau_a(Sa_S)
            * c_r_rear
            / (self.Srear * self.brear)
            * (
                0.5 * (b_2rear**2 - b_1rear**2)
                + 2 * (self.taper - 1) / (3 * self.brear) * (b_2rear**3 - b_1rear**3)
            )
            * self.eta_rear
        )
    ...
    def Clp(self):
        ...
        Clp_rear = (
            -(self.Clarear + self.Cd0rear)
            * c_r_rear
            * self.brear
            / (24 * self.Srear)
            * (1 + 3 * self.taper)
            * self.eta_rear ** (1 - 1.38 / self.bfwd)
        )
        Clp = Clp_fwd * self.Sfwd * self.bfwd / (
            self.S * b
        ) + Clp_rear * self.Srear * self.brear / (self.S * b)
        return Clp
    ...
    def plotting(self, Sa_S, b1, b2, Se_S, be_b, rear):
        if isinstance(Sa_S, (float, int)) and not isinstance(b2, (float, int)):
            ...
            ya_1 = abs(
                b1 / 100 * self.bfwd / 2 * np.tan(self.Sweep(self.Afwd, 0, 100, 25))
            )
            xa_2 = b2 / 100 * self.bfwd / 2
            ya_2 = abs(
                b2 / 100 * self.bfwd / 2 * np.tan(self.Sweep(self.Afwd, 0, 100, 25))
            )
            #### Aileron geometry ####
