...
class Stab_Derivatives:
    def __init__(
        ...
        self.CLafwd, self.CLarear = (
            CLafwd,
            CLarear,
        )  # Wing lift curve slopes for both wings [1/rad]
        ...
        self.Cl0fwd = (
            CLfwd0 / (np.cos(self.Sweep(self.Afwd, self.taper, 0, 25, 0))) ** 2
        )  # Airfoil lift at zero angle of attack
        self.Cl0rear = (
            CLrear0 / (np.cos(self.Sweep(self.Arear, self.taper, 0, 25, 0))) ** 2
        )  # Airfoil lift at zero angle of attack
        self.d = d
        self.dy = dy
        ...
        self.b = np.sqrt(
            0.5
            * (self.Srear / self.S * self.Arear + self.Sfwd / self.S * self.Afwd)
            * self.S
        )
    ...
    def deps_da(self, Lambda_quarter_chord, h_ht, CLaw, eta=0.5):
        ...
        Keps = (
            (0.1124 + 0.1265 * Lambda_quarter_chord + 0.1766 * Lambda_quarter_chord**2)
            / r**2
            + 0.1024 / r
            + 2
        )
        ...
        if (
            180 / np.pi * phi < 30 and 180 / np.pi * phi > 0
        ):  # To account for propeller downwash
            dsde_da = (
                6.5
                * (
                    self.rho
        ...
        value = (
            ...
            * A
            / (
                ...
                + np.sqrt(
                    4
                    + ((A * beta / eta) ** 2) * (1 + (np.tan(Lambda_half) / beta) ** 2)
                )
            )
        )
        return value
    ...
    def u_derivatives(self):
        ...
        Cm_M = (
            CLfwd_M * (self.xcg - self.xacfwd) * self.Sfwd / (self.S * self.c)
            - CLrear_M
            * (self.xacrear - self.xcg)
            * self.Srear
            / (self.S * self.c)
            * self.eta_rear
        )
    ...
    def alpha_derivatives(self):
        ...
        CDa = (
            CDafwd * self.Sfwd / self.S
            + CDarear * (1 - de_da) * self.Srear / self.S * self.eta_rear
        )
        CLa = (
            self.CLafwd * self.Sfwd / self.S
            + self.CLarear * (1 - de_da) * self.Srear / self.S * self.eta_rear
        )
        CX_a = -CDa + CLa * self.alpha0 + self.CL0
        CZ_a = -CDa * self.alpha0 - self.CD0 - CLa
        Cm_a = (
            -CDafwd * (self.zcg - self.dy) * self.Sfwd / (self.S * self.c)
    ...
    def q_derivatives(self):
        ...
        Cm_q = -(
            self.CLafwd
            * (self.xcg - self.xacfwd) ** 2
            * self.Sfwd
            / (self.S * self.c**2)
            + self.CLarear
            * (self.xacrear - self.xcg) ** 2
            * self.Srear
            / (self.S * self.c**2)
            * self.eta_rear
        )
    ...
    def alpha_dot_derivatives(self):
        ...
        CX_adot = 0
        darear_dadot = (
            self.deps_da(self.Sweepc4, self.hfus - self.dy, self.CLafwd)
            * self.lh_arm()
            / self.c
        )
    ...
    def r_derivatives(self):
        ...
        CY_r = (
            2
            * self.C_L_a(self.ARv, self.Sweep(self.ARv * 2, 0.4, self.sweepTE, 50, 100))
            * (self.lv)
            * self.Sv
            / (self.S * self.b)
            * self.eta_v
        ...
        Cl_r = (
            Cl_r_v
            + Cl_r_fwd * self.Sfwd * self.bfwd / (self.S * self.b)
        ...
        Cn_r_wing = (
            ...
            / 4
            * (
                CDfwd0 * self.Sfwd * self.bfwd / (self.S * self.b)
                + CDrear0 * self.Srear * self.brear / (self.S * self.b) * self.eta_rear
            )
        )
    ...
    def p_derivatives(self):
        ...
        CY_p = (
            ...
            / (3 * np.pi)
            * eta_v
            * (self.bv * self.Sv / (self.S * self.b))
            * self.C_L_a(self.ARv, self.Sweep(self.ARv * 2, 0.4, self.sweepTE, 50, 100))
        )
        Cn_p_v = -(self.lv) / (self.b) * CY_p
        c_r_fwd = (
            self.cfwd * 3 / 2 * (1 + self.taper) / (1 + self.taper + self.taper**2)
        )
        c_r_rear = (
            self.crear * 3 / 2 * (1 + self.taper) / (1 + self.taper + self.taper**2)
        )
        Cnp_fwd = (
            -(self.Cl0fwd)
            * c_r_fwd
        ...
        Cn_p_wings = (
            ...
            / 8
            * (
                self.CLfwd0 * self.Sfwd * self.bfwd / (self.S * self.b)
                + self.CLrear0 * self.Srear * self.brear // (self.S * self.b)
            )
        )
        Cn_p_wings_analytical = Cnp_fwd * self.Sfwd * self.bfwd / (
            self.S * self.b
        ) + Cnp_rear * self.Srear * self.brear / (self.S * self.b)
    ...
    def beta_derivatives(self, g1, g2):
        ...
        CY_b = (
            -self.C_L_a(
                self.ARv, self.Sweep(self.ARv * 2, self.taper_v, self.sweepTE, 50, 100)
            )
            * (1 - dsigma_dbeta)
            * self.eta_v
            * self.Sv
        ...
        Cnb_w_fwd = self.CLfwd0**2 * (
            1 / (4 * np.pi * self.Afwd)
            - (np.tan(self.Sweepc4) / (np.pi * self.Afwd + 4 * np.cos(self.Sweepc4)))
            * (
                np.cos(self.Sweepc4)
                - self.Afwd / 2
                - self.Afwd**2 / (8 * np.cos(self.Sweepc4))
                - 6
                * (self.xacfwd - self.xcg)
                * np.sin(self.Sweepc4)
                / (self.Afwd * self.c)
            )
        )
        Cnb_w_rear = self.CLrear0**2 * (
            1 / (4 * np.pi * self.Arear)
            - (
                np.tan(self.Sweepc4_rear)
                / (np.pi * self.Arear + 4 * np.cos(self.Sweepc4_rear))
            )
            * (
                np.cos(self.Sweepc4_rear)
                - self.Afwd / 2
                - self.Arear**2 / (8 * np.cos(self.Sweepc4_rear))
                - 6
                * (self.xacrear - self.xcg)
                * np.sin(self.Sweepc4_rear)
                / (self.Arear * self.c)
            )
        )
        Cn_b_wings = (
            Cnb_w_fwd * self.Sfwd * self.bfwd / (self.S * bmax)
            + Cnb_w_rear * self.Srear * self.brear / (self.S * bmax) * self.eta_rear
        )
        ...
        # Cl_b = -0.110
        Cl_b_fwd = (
            -self.CLafwd * g1 / 4 * (2 / 3 * (1 + 2 * self.taper) / (1 + self.taper))
        )
        Cl_b_rear = (
            -self.CLarear
            * g2
            / 4
            * (2 / 3 * (1 + 2 * self.taper) / (1 + self.taper))
            * self.eta_rear
        )
        # print(self.CLafwd,self.Gamma_fwd,self.taper)
        Cl_b = (
            Cl_b_wf_rear * self.Srear * self.brear
            + Cl_b_wf_fwd * self.Sfwd * self.bfwd
            + Cl_b_fwd * self.Sfwd * self.bfwd
            + Cl_b_rear * self.Srear * self.brear
        ) / (self.S * self.b) + Cl_b_v
    ...
    def de_derivatives(self, Se_S, be_b):
        ...
        CL_de = (
            -self.CLafwd * self.tau_e(Se_S) * be_b / 100 * self.Sfwd / self.S
            + self.CLarear
            * self.tau_e(Se_S)
            * be_b
            / 100
        ...
        CZ_de = -CL_de
        Cm_de = -(
            CL_de_fwd * (self.xcg - self.xacfwd) / self.c
            + CL_de_rear * (self.xacrear - self.xcg) / self.c
        )
    ...
    def asym_stability_req(self, Ixx, Izz, Ixz, Sa_S, b1, b2, cr_cv, br_bv, g1, g2):
        ...
        C_L = self.CL0
        C_Y = [
            self.beta_derivatives(0, 0)[0],
            0,
            self.p_derivatives()[0],
            self.r_derivatives()[0],
            self.da_derivatives(Sa_S, b1, b2)[0],
            self.dr_derivatives(cr_cv, br_bv)[0],
        ]
        C_l = [
            self.beta_derivatives(0, 0)[1],
            self.p_derivatives()[1],
            self.r_derivatives()[1],
            self.da_derivatives(Sa_S, b1, b2)[1],
            self.dr_derivatives(cr_cv, br_bv)[1],
        ]
        C_l_b_1 = self.beta_derivatives(g1, 0)[1]
        C_l_b_2 = self.beta_derivatives(0, g2)[1]
        C_l_b_opt = self.beta_derivatives(g1, g2)[1]
        C_n = [
            self.beta_derivatives(0, 0)[2],
            0,
            self.p_derivatives()[2],
            self.r_derivatives()[2],
            self.da_derivatives(Sa_S, b1, b2)[2],
            self.dr_derivatives(cr_cv, br_bv)[2],
        ]
        ...
        C = (
            ...
            * mu_b
            * (
                (C_y_b * C_n_r - C_y_r * C_n_b) * Kxx2
                + (C_y_b * C_l_p - C_l_b * C_y_p) * Kzz2
                + ((C_y_b * C_n_p - C_n_b * C_y_p) + (C_y_b * C_l_r - C_l_b * C_y_r))
                * Kxz
                + 4 * mu_b * C_n_b * Kxx2
                + 4 * mu_b * C_l_b * Kxz
        ...
        D = (
            -4 * mu_b * C_L * (C_l_b * Kzz2 + C_n_b * Kxz)
            + 2 * mu_b * (C_l_b * C_n_p - C_n_b * C_l_p)
            + 0.5 * C_y_b * (C_l_r * C_n_p - C_n_r * C_l_p)
            + 0.5 * C_y_p * (C_l_b * C_n_r - C_n_b * C_l_r)
            + 0.5 * C_y_r * (C_l_p * C_n_b - C_n_p * C_l_b)
        )
        ...
        def C_ER(clb, cnb):
            return (
                ...
                * mu_b
                * (
                    (C_y_b * C_n_r - C_y_r * cnb) * Kxx2
                    + (C_y_b * C_l_p - clb * C_y_p) * Kzz2
                    + ((C_y_b * C_n_p - cnb * C_y_p) + (C_y_b * C_l_r - clb * C_y_r))
                    * Kxz
                    + 4 * mu_b * cnb * Kxx2
                    + 4 * mu_b * clb * Kxz
                    + 0.5 * (C_l_p * C_n_r - C_n_p * C_l_r)
        ...
        def D_ER(clb, cnb):
            return (
                -4 * mu_b * C_L * (clb * Kzz2 + cnb * Kxz)
                + 2 * mu_b * (clb * C_n_p - cnb * C_l_p)
                + 0.5 * C_y_b * (C_l_r * C_n_p - C_n_r * C_l_p)
                + 0.5 * C_y_p * (clb * C_n_r - cnb * C_l_r)
                + 0.5 * C_y_r * (C_l_p * cnb - C_n_p * clb)
            )
        ...
        plt.plot(
            clb2, cnb_E(-clb2), color="k", label="Limit for Spiral stability when E = 0"
        )
        plt.xlabel(r"-$C_{l_{\beta}}$ [rad$^{-1}$]", fontsize=12)
        plt.ylabel(r"$C_{n_{\beta}}$ [rad$^{-1}$]", fontsize=12)
        plt.scatter(
            -C_l_b, C_n_b, color="k", marker="x", label="Wigeon with no dihedral"
        )
        plt.scatter(
            -C_l_b_1,
            C_n_b,
            color="r",
            marker="o",
            label=r"Wigeon with $\Gamma_{fwd} = %.1f$ deg " % (np.rad2deg(g1)),
        )
        plt.scatter(
            -C_l_b_2,
            C_n_b,
            color="b",
            marker="X",
            label=r"Wigeon with $\Gamma_{rear} = %.1f$ deg" % (np.rad2deg(g2)),
        )
        plt.scatter(
            -C_l_b_opt,
            C_n_b,
            color="g",
            marker="D",
            label=r"Wigeon with both $\Gamma_{fwd} = %.1f$ deg and $\Gamma_{rear} = %.1f$ deg"
            % (np.rad2deg(g1), np.rad2deg(g2)),
        )
        plt.legend(loc=1)
        plt.show()
        return
    ...
    def initial_coeff(self):
        C_X0 = (
            ...
            * np.sin(self.th0 + self.alpha0)
        )
        C_Z0 = (
            -self.W
            / (0.5 * self.rho * self.V0**2 * self.S)
            * np.cos(self.th0 + self.alpha0)
        )
        C_m0 = 0
        return C_X0, C_Z0

    def return_stab_derivatives(
        self,
        Se_S,
        be_b,
        Sa_S,
        ...
        C_x_a, C_x_a_dot, C_x_u, C_x_0, C_x_q, C_x_d = (
            a[0],
            a_dot[0],
            u[0],
            initial[0],
        ...
        C_z_a, C_z_a_dot, C_z_u, C_z_0, C_z_q, C_z_d = (
            a[1],
            a_dot[1],
            u[1],
            initial[1],
        ...
        # print(C_X, C_Z, C_m)
        C_y_b, C_y_b_dot, C_y_p, C_y_r, C_y_da, C_y_dr = (
            b[0],
            0,
            p[0],
            r[0],
        ...
        C_n_b, C_n_b_dot, C_n_p, C_n_r, C_n_da, C_n_dr = (
            b[2],
            0,
            p[2],
            r[2],
