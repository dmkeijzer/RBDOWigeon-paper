...
class initial_sizing:
    def __init__(
        ...
        self.CD_vertical = (
            1  # Drag polar for the fuselage at 90 degrees TODO: IMPLEMENT
        )
        ...
        self.eff_prop = prop[
            "eff_cruise"
        ]  # [-] Propeller efficiency during normal flight
    ...
    def wing_loading(self):
        ...
        self.ax1.plot(
            self.WS, self.WP_turn_max_speed, label="Turn performance @ $V_{max}$"
        )
        ...
        self.ax1.plot(
            np.ones(np.size(self.WS)) * self.WS_stall,
            np.linspace(0, 0.1, np.size(self.WS)),
            label="Stall speed",
        )
    ...
    def design_point(self):
        ...
        crit = np.minimum(
            np.minimum(
                np.minimum(
                    np.minimum(self.WP_speed, self.WP_turn_cruise),
                    self.WP_turn_max_speed,
                ),
                self.WP_climb,
            ),
            self.WP_hov,
        ...
        crit_cruise = np.minimum(
            np.minimum(
                np.minimum(self.WP_speed, self.WP_turn_cruise), self.WP_turn_max_speed
            ),
            self.WP_climb,
        )[self.WS < self.WS_stall]
        ...
        self.ax1.fill_between(
            ws_crit, np.zeros(np.size(crit)), crit, facecolor="green", alpha=0.2
        )
        self.ax1.fill_between(
            ws_crit,
            np.zeros(np.size(crit_cruise)),
            crit_cruise,
            facecolor="limegreen",
            alpha=0.2,
        )
...
class mission_analysis:
    def __init__(self, path, h_cruise, m_pl, ac_energy, concept, save_data=False):
        ...
        # Descent angle
        self.gamma_descent = np.arctan(
            CL[idx] / CD[idx]
        )  # np.radians(self.FP["Gamma descent"])
    ...
    def hover_power(self, ROC, W):
        ...
        P_hov = T + 1.2 * T * (
            -ROC / 2 + np.sqrt(ROC**2 / 4 + T / (2 * self.rho_sl * A_prop))
        )
    ...
    def cruise_power(self, W):
        ...
        CDCL_cr = (
            self.CDmin + (((self.CL_range - self.CLmin) ** 2) * self.k)
        ) / self.CL_range
    ...
    def descent_power(self, gamma_descend, V_desc, W):
        ...
        P_des = np.maximum(
            W * V_desc * ((CLCD_cl**-1) - np.sin(gamma_descend)) / self.cruise_eff, 0
        )

        if np.any(P_des < 0):
            print(
                "Descent power implemented incorrectly or descent angle too steep to maintain speed (use brakes/HLD)"
            )
    ...
    def cruise_energy(self, mission_range):
        ...
        if np.any(d_cruise < 0):
            print(
                "Cruise distance needed for climb and descent longer than mission time, reduce cruise altitude"
            )
            ...
            np.minimum(
                (
                    (
                        (
                            (
                                self.WP_cr * dedicated_hover
                                + self.WP_ho * (not dedicated_hover)
                            )
                            ** -1
                        )
                        * self.cruise_eff
    ...
    def climb_perf_chart(self):
        ...
        plt.tight_layout()
        path = (
            "C:/Users/Egon Beyne/Desktop/DSE/Plots/climb_perf_"
            + str(self.concept)
            + ".pdf"
        )
        plt.savefig(path)
    ...
    def total_energy(self, mission_range, pie=False):
        ...
        if pie:
            ...
            plt.tight_layout()
            path = (
                "C:/Users/Egon Beyne/Desktop/DSE/Plots/Energy_breakdown_"
                + str(self.concept)
                + ".pdf"
            )
