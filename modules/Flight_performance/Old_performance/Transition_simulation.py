...
class transition_EOM:
    ...
    def disk_power(self, T, P_in, a_T, V):
        ...
        P = (
            ...
            + self.k
            * T
            * (-V_perp / 2 + np.sqrt(V_perp**2 / 4 + T / (2 * self.rho * self.A_prop)))
            - P_in
        )
    ...
    def simulate(self, plotting=False):
        ...
        t = 0.0
        dt = 0.1
        vx = 0.0  # Horizontal speed
        vy = 0.0  # Vertical speed
        x = 0.0  # np.array([[0], [0]], dtype = 'float64')     # horizontal position
