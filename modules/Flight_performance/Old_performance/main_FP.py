...
Energy_analysis = mission_analysis(
    data_path, cruising_alt, 360, energy, concept, save_data=True
)
...
plt.fill_between(
    ranges,
    np.ones(np.size(tot_weight)) * empty_weight,
    np.zeros(np.size(tot_weight)),
    color="red",
    alpha=0.5,
)
plt.fill_between(
    ranges,
    np.ones(np.size(tot_weight)) * max_weight,
    np.ones(np.size(tot_weight)) * 1e9,
    color="red",
    alpha=0.5,
)
