...
class sensitivity_analysis:
    ...
    def cruising_altitude(self):
        ...
        for i, h in enumerate(altitude):
            ...
            dist[i], time[i] = performance.range(
                cruising_altitude=h, cruise_speed=self.V_cruise, mass=self.MTOM
            )
    ...
    def cruise_speed(self, plotting=True):
        ...
        for i, V_cr in enumerate(cruise_speeds):
            print(i)
            performance = evtol_performance(
                cruising_alt=self.h_cruise_opt,
                cruise_speed=V_cr,
                S=self.S,
                CL_max=self.CL_max,
                mass=self.MTOM,
            ...
            dist[i], time[i] = performance.range(
                cruising_altitude=self.h_cruise_opt, cruise_speed=V_cr, mass=self.MTOM
            )
    ...
    def wind(self, test_wind=72, testing=False, plotting=True):
        ...
        for i, wind in enumerate(winds):
            ...
            dist[i], time[i] = performance.range(
                cruising_altitude=self.h_cruise_opt,
                cruise_speed=self.V_cruise,
                mass=self.MTOM,
                wind_speed=wind,
            )
