...
class mission:
    def __init__(
        ...
        self.A_disk = A_disk
        self.P_max = P_max
        self.plotting_monte_carlo = plot_monte_carlo
    ...
    def target_accelerations_new(
        self, vx, vy, y, y_tgt, vx_tgt, max_ax, max_ay, max_vy, h_trans
    ):
        ...
        if (
            h_trans + (np.abs(vy) / self.ay_target_descend) > y > y_tgt
            and abs(vx) > 0.25
        ):
            vy_tgt = 0
    ...
    def numerical_simulation(self, vx_start, y_start, th_start, y_tgt, vx_tgt, h_trans):
        ...
        while running:
            ...
            ax_tgt, ay_tgt = self.target_accelerations_new(
                vx, vy, y, y_tgt, vx_tgt, max_ax, max_ay, max_vy, h_trans=h_trans
            )
    ...
    def single_sample_monte_carlo(
        self, d_total, t_loit_cruise, h_trans, h_loiter_cruise, t_loit_hover, h_cruise
    ):
        ...
        if d_cruise < 0:
            ...
            logging.info(
                f"Encountered a negative cruise distance for a total travel distance of {d_total}"
            )
        ...
        P_cruise, D_cruise = self.power_cruise_config(
            h_cruise, self.v_cruise, self.m
        )  # + self.P_systems

        V = speeds(
            altitude=const.h_cruise,
            m=self.m,
            CLmax=self.CL_max,
            S=self.S,
        ...
        P_loiter, _ = self.power_cruise_config(
            altitude=h_loiter_cruise, speed=V_loit, mass=self.m
        )  # + self.P_systems
        ...
        energy_dist = np.array(
            [E_cruise, E_climb, E_desc, E_loiter_cruise, E_loiter_hover]
        )
