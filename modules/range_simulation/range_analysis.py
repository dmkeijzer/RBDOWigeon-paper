...
df_trips = (
    pd.read_csv(path_range_data).to_numpy()
)  # array - example row ['The Hague-Bremen' 326.5480004995904 'The-Bre-dir' 99.0]
...
class Bin:
    def __init__(self, frequency, edges, flight_df, total_df_gdp):
        ...
        self.flights = flight_df[
            (flight_df[:, 1].astype("float64") >= self.left_bound)
            * (flight_df[:, 1].astype("float64") < self.right_bound)
        ]
        self.freq = frequency
            ...
            for idx_j, j in enumerate(labels):
                if idx_i != idx_j:
                    if i == j:
                        eq_par_trip.append(
                            [
                                self.flights[idx_i, 0],
                                self.flights[idx_j, 0],
                                self.flights[idx_i, 3],
                            ]
                        )
        eq_par_trip = np.array(eq_par_trip)
        if len(eq_par_trip) > 0:
            corr_fac = (
                np.sum(eq_par_trip[::2, 2].astype("float64")) / self.df_gdp
            )  # corrrection factor for rule of addition, P(A n B) happens when a two way trip had the same parent trip
...
def iso_cities(lim):
    ...
    for idx_i, city in enumerate(fc.df_geo[:, 0]):
        ...
        for idx_j, trip in enumerate(df_trips[:, 0]):
            if (
                trip.find(city) != -1
            ):  # checks whether city from outer loop is in the trip from the inner loop
                loc.append(idx_j)
        ...
        all_trips_check = (
            df_trips[loc, 1] < lim
        )  # list of all trips containing the city from outer loop
...
def two_trip_analysis(lim):
    ...
    two_trip = df_trips[
        (df_trips[:, 1] < 2 * lim) * (df_trips[:, 1] > lim)
    ]  # selects trips which could be flown in two flight but not one
    one_trip = df_trips[df_trips[:, 1] < lim]  # array - selects direct flights

    for trip_2 in two_trip:
        depar_connect = np.ones(
            (1, 4)
        )  # array - initialize arrays which will contain all direct connections to the current hub in the loop
        ...
        for (
            i
        ) in one_trip:  # loop to get all connections from departure hub under the limit
        ...
        depar_connect = np.delete(
            depar_connect, 0, axis=0
        )  # deleting ones created at the beginning to avoid error
        dest_connect = np.delete(dest_connect, 0, axis=0)

        hubs_depar = [
            i.split("-")[1] if i.split("-")[0] == depar else i.split("-")[0]
            for i in depar_connect[:, 0]
        ]  # creating list with connected hubs so we math them
        hubs_dest = [
            i.split("-")[1] if i.split("-")[0] == dest else i.split("-")[0]
            for i in dest_connect[:, 0]
        ]

        if (
            len(set(hubs_dest).intersection(hubs_depar)) == 0
        ):  # bool - See if a match exists between connected hubs, if not continue to next trip
            continue
        else:
            intersections = set(
                hubs_dest
                ...
                hubs_depar
            )  # set - intersection refers to the hubs both departure and destination could fly to
            lst_depar_intersec = []  # list - initalize list that will contain distance fro departure to intersection
            lst_dest_intersec = []

            for idx in range(
                len(intersections)
            ):  # this loop gets the distances from all possible routes so  from departure - intersections and intersection - destination
                lst_depar_intersec.append(
                    [
                        i[1]
                        for i in one_trip
                        if depar + "-" + list(intersections)[idx] == i[0]
                        or list(intersections)[idx] + "-" + depar == i[0]
                    ][0]
                )
                lst_dest_intersec.append(
                    [
                        i[1]
                        for i in one_trip
                        if dest + "-" + list(intersections)[idx] == i[0]
                        or list(intersections)[idx] + "-" + dest == i[0]
                    ][0]
                )

            sum_range = np.array(lst_depar_intersec) + np.array(
                lst_dest_intersec
            )  # array - sum the departure - intersection - destination ranges
            idx_intersec = np.where(sum_range == min(sum_range))[0][
                0
            ]  # integer - represents the index
            shorterst_intersec = list(intersections)[
                idx_intersec
            ]  # string - city shortest intersectiom
            label = depar[:3] + "-" + dest[:3] + "-2step"
            sum_gdp = trip_2[
                3
            ]  # Get the sum gdp, which will be necessary for the probability distribution
            data.append(
                [
                    depar + "-" + shorterst_intersec,
                    float(lst_depar_intersec[idx_intersec]),
                    label,
                    sum_gdp,
                ]
            )
            data.append(
                [
                    shorterst_intersec + "-" + dest,
                    float(lst_dest_intersec[idx_intersec]),
                    label,
                    sum_gdp,
                ]
            )
    ...
    data = np.array(
        data
    )  # array - example of a row ['Lyon-Milan', '342.78286276713493', 'Par-Mil-2step', '955.8']
    return data
...
def plot_hist_two_trip_weightless(lim, n_bins):
    ...
    if np.size(two_step_flights) != 0:
        total_flights = np.append(
            direct_flights, two_step_flights, axis=0
        )  # array - example ['Paris-Dortmund', 470.1237855871516, 'Par-Dor-dir', 910.4]
    else:
        total_flights = direct_flights
    hist_data = total_flights[:, 1].astype("float64")
    bfd = pf.BestFitDistribution(pd.DataFrame(hist_data))
    bfd.analyze(
        title="Two trip distribution weightless",
        x_label="range",
        y_label="freq",
        allBins=n_bins,
        outputFilePrefix=f"../distr_fig/two_trip_fit_weightless_"
        + "range_"
        + str(lim)
        + "_bins_"
        + str(n_bins),
...
def plot_hist_two_trip_weights(lim, n_bins=9):
    ...
    hist_data = total_flights[:, 1].astype("float64")
    if np.size(two_step_flights) != 0:
        df_sum_gdp = np.sum(direct_flights[:, 3].astype("float64")) + np.sum(
            two_step_flights[:, 3].flatten().astype("float64")[::2]
        )
    else:
    ...
    for idx, i in enumerate(bin_edges):
        ...
        state_bin = Bin(
            state_freq, state_edges, total_flights, df_sum_gdp
        )  # Use created class to do computations on the bins
        corr_bins.append(state_bin)

    raw_data_expon_fit = [
        list(np.linspace(i.left_bound, i.right_bound, ceil(i.bin_expectation())))
        for i in corr_bins
    ]

    flat_raw_data_expon = np.array(
        [item for sublist in raw_data_expon_fit for item in sublist]
    )  # flattening using list comprehensino nparray.flatten() did not work
