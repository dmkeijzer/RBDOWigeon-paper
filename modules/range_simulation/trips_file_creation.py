...
path_geo_dataset = os.path.join(home_dir, r"ne_10m_populated_places_simple.shp")
path_cities_dataset = os.path.join(home_dir, r"cities_by_gmp.xlsx")
path_cities_gdp_coord = os.path.join(home_dir, r"plotting_df.csv")
...
correction_data = [
    [
        ...
        51.509865,
        -0.118092,
    ],  # Here they are added back again with the correct coordinates
...
def create_trip_file():
    ...
    for idx_i, row_i in enumerate(city_coord_gdp.to_numpy()):
        ...
        for idx_j, row_j in enumerate(city_coord_gdp.to_numpy()[n:, :], start=n):
            ...
            journey = i_str + "-" + j_str
            dist_journey = distance(
                i_coord, j_coord
            ).km  # computes distance based on coordinates
            sum_gdp = row_i[4] + city_coord_gdp_arr[idx_j, 4]
            label = (
                i_str[:3] + "-" + j_str[:3] + "-dir"
            )  # give label important for last computational step
    ...
    df_distance.to_csv(
        r"C:\Users\damie\OneDrive\Desktop\Damien\Wigeon_proj\own_code\trip_2.0.csv",
        header=["city-city", "dist", "label", "sum_gdp"],
        index=False,
    )
