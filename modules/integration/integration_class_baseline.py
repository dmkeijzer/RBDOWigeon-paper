...
class RunDSE:
    ...
    def run(self, internal_inputs):
        ...
        DragClass = drag_comp.componentdrag(
            "tandem",
            S_tot,
            const.l_nosecone,
            const.l_cylinder,
        ...
        CD_a_w = wing_design.CDa_poststall(
            const.tc, CDs, CDs_f, Afus, alpha_lst, "wing", DragClass.CD, de_da
        )
        CD_a_f = wing_design.CDa_poststall(
            const.tc, CDs, CDs_f, Afus, alpha_lst, "fus", DragClass.CD, de_da
        )

        # Energy sizing
        MissionClass = FP.mission(
            MTOM,
            h_cr,
            V_cr,
            CLmax,
        ...
        energy, t_tot, P_max_eng_mission, max_thrust, t_hor, energy_segments = (
            MissionClass.total_energy(simplified=False)
        )
        ...
        energy = (
            energy * 2.77778e-7 * 1000 * const.energy_cont / eff_overall
        )  # From [J] to [Wh]
        ...
        P_max_control = MissionClass.thrust_to_power(
        ...
        (
            ...
            Sv,
            root_chord_vtail,
            contingency=const.baseline_contingency,
            cg_bat=cg_bat,
        )
