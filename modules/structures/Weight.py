...
class Vtail:
    def __init__(self, mtom, Sv, Av, rchord, toc, sweep4_deg):
        ...
        self.mass = (
            (1.68 * self.mtom_lbs**0.567 * self.Sv_ft**1.249 * self.Av**0.482)
            / (639.95 * self.trv**0.747 * np.cos(self.sweep) ** 0.882)
        ) * 0.453592
...
class Wing:
    ...
    def __init__(self, mtom, S1, S2, n_ult, A_1, A_2, pos=[], wmac=0.8, toc=0.17):
        self.S1_ft, self.S2_ft, self.S1, self.S2 = (
            S1 * 3.28084**2,
            S2 * 3.28084**2,
            S1,
            S2,
...
class Weight:
    def __init__(
        ...
        self.wing, self.fuselage, self.landing_gear, self.prop = (
            wing,
            fuselage,
            landing_gear,
            propulsion,
        ...
        self.wmass, self.fmass, self.lmass, self.pmass = (
            np.sum(self.wing.mass),
            self.fuselage.mass,
        ...
        if contingency:
            self.mtom = (
                self.wmass * const.mass_cont
                + self.pmass * const.mass_cont
                + self.lmass * const.mass_cont
                + self.fmass * const.mass_cont
                + self.cmass
                + self.bmass * const.mass_cont
                + self.tot_m_pax
                + self.vmass * const.mass_cont
            )
            self.mtom_cg = (
                (
                    self.moment_w * const.mass_cont
                    + self.moment_p * const.mass_cont
                    + self.moment_l * const.mass_cont
                    + self.moment_f * const.mass_cont
                    + self.moment_c
                    + self.moment_b * const.mass_cont
                    + self.moment_pax
                    + self.moment_v * const.mass_cont
                )
                / self.mtom
            ...
            print(
                "Line 114 - Weight.py - testing NC", self.moment_l
            )  # (self.moment_w + self.moment_p + self.moment_l + self.moment_f + self.moment_c + self.moment_b + self.moment_pax + self.moment_v))
    ...
    def print_weight_fractions(self):
        ...
        d["Front wing"] = [
            self.wing.mass[0],
            self.wing.mass[0] / self.oem,
            self.wing.mass[0] / self.mtom,
        ]
        d["Back wing"] = [
            self.wing.mass[1],
            self.wing.mass[1] / self.oem,
            self.wing.mass[1] / self.mtom,
        ]
        ...
        print("")
        print(
            f"Where OEM is {self.oem}kg with CG of {self.oem_cg}m, and MTOM is {self.mtom}kg with CG of {self.mtom_cg}m"
        )
        # for key in d:
    ...
    def MMI(self, wmac=[], toc=[], vpos_wing=[]):
        ...
        fus_mmi_y = (
            self.fmass
            * (self.fuselage.lf**2 + 3 * (self.fuselage.wf / 2) ** 2 + 3 * irad**2)
            / 12
        )
        fus_mmi_z = fus_mmi_y
        ...
        wing1_mmi_x, wing1_mmi_y, wing1_mmi_z = (
            self.wing.mass[0] * (span1**2 + t1**2) / 12,
            self.wing.mass[0] * (span1**2 + wmac[0] ** 2) / 12,
        ...
        wing2_mmi_x, wing2_mmi_y, wing2_mmi_z = (
            self.wing.mass[1] * (span2**2 + t2**2) / 12,
            self.wing.mass[1] * (span2**2 + wmac[1] ** 2) / 12,
            self.wing.mass[1] * (wmac[1] ** 2 + t2**2) / 12,
        )

        # passengers - modeled as a prism 2.2 x 1.1 x 1 m (length x width x height)
        pld_mmi_x, pld_mmi_y, pld_mmi_z = (
            self.tot_m_pax * (1.1**2 + 1**2) / 12,
            self.tot_m_pax * (1.1**2 + 2.2**2) / 12,
            self.tot_m_pax * (2.2**2 + 1**2),
        )

        # propulsion - modeled as a solid cylinder
        m_prop = self.pmass / self.prop.nprop
        lprop, rprop = 0.4, 0.12
        prop_mmi_x, prop_mmi_y, prop_mmi_z = (
            m_prop * (rprop**2) / 2,
            m_prop * (lprop**2 + 3 * rprop**2) / 12,
            m_prop * (lprop**2 + 3 * rprop**2) / 12,
        )

        # battery - modeled as a prism
        lbat, tbat, wbat = 0.4 * self.fuselage.lf, 0.2, 1
        bat_mmi_x, bat_mmi_y, bat_mmi_z = (
            self.bmass * (wbat**2 + tbat**2) / 12,
            self.bmass * (wbat**2 + lbat**2) / 12,
            self.bmass * (tbat**2 + lbat**2) / 12,
        )
        span = span2
        oem_mmi_x = (
            fus_mmi_x
            + (wing1_mmi_x + self.wing.mass[0] * (0.7 - vpos[0]) ** 2)
            + (wing2_mmi_x + self.wing.mass[1] * (vpos[1] - 0.7) ** 2)
            + 4
            * np.sum(
                m_prop
                * (
                    np.sqrt(
                        (0.7) ** 2
                        + (np.linspace(0.9, span1 / 2, int(self.prop.nprop / 4))) ** 2
                    )
                )
                ** 2
            )
            + self.prop.nprop * prop_mmi_x
            + bat_mmi_x
            + pld_mmi_x
        )
        ...
        oem_mmi_y = (
            fus_mmi_y
            + (wing1_mmi_y + self.wing.mass[0] * (self.mtom_cg - self.wing.pos1) ** 2)
            + (wing2_mmi_y + self.wing.mass[1] * (self.wing.pos2 - self.mtom_cg) ** 2)
            + 4
            * np.sum(
                m_prop
                * (
                    np.sqrt(
                        (self.wing.pos2 - 0.7) ** 2
                        + (np.linspace(0.9, span / 2, int(self.prop.nprop / 4))) ** 2
                    )
                )
                ** 2
            )
            + self.prop.nprop * prop_mmi_y
            + bat_mmi_y
            + self.bmass * (self.mtom_cg - self.battery_pos) ** 2
            + pld_mmi_y
            + self.tot_m_pax * ((lf / 2 - 0.5) - self.mtom_cg) ** 2
        )
        ...
        oem_mmi_z = (
            ...
            + (
                wing1_mmi_z
                + self.wing.mass[0]
                * (np.sqrt((0.7 - vpos[0]) ** 2 + (self.mtom_cg - self.wing.pos1) ** 2))
                ** 2
            )
            + (
                wing2_mmi_z
                + self.wing.mass[1]
                * (np.sqrt((vpos[1] - 0.7) ** 2 + (self.wing.pos2 - self.mtom_cg) ** 2))
                ** 2
            )
            + bat_mmi_z
            + self.bmass * (self.mtom_cg - self.battery_pos) ** 2
            + (
                ...
                * (
                    (
                        np.sqrt(
                            (0.7 - vpos[0]) ** 2 + (self.mtom_cg - self.wing.pos1) ** 2
                        )
                    )
                    ** 2
                )
            ...
            * self.prop.nprop
            ...
            + (
                ...
                * (
                    (
                        np.sqrt(
                            (vpos[1] - 0.7) ** 2 + (self.wing.pos2 - self.mtom_cg) ** 2
                        )
                    )
                    ** 2
                )
        ...
        oem_mmi_xy = (
            (self.wing.mass[0] * (0.7 - vpos[0]) * (self.mtom_cg - self.wing.pos1))
            + self.prop.nprop
            * (m_prop * (0.7 - vpos[0]) * (self.mtom_cg - self.wing.pos1))
            / 2
            + (self.wing.mass[1] * (vpos[1] - 0.7) * (self.wing.pos2 - self.mtom_cg))
            + self.prop.nprop
            * (m_prop * (vpos[0] - 0.7) * (self.wing.pos2 - self.mtom_cg))
            / 2
        )
...
if __name__ == "__main__":
    ...
    m_prop = [
        576.2165134804536 / 12
    ] * 12  # list of mass of engines (so 30 kg per engine with nacelle and propeller)
    pos_prop = [
        -0.02401748,
        -0.02401748,
        -0.02401748,
    ...
    Ixx, Iyy, Izz, Ixz = weight.MMI(
        wmac=[0.7, 0.8], toc=[0.17, 0.17], vpos_wing=[0.3, 1.6]
    )
