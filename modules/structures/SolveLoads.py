...
class Engines:
    ...
    __repr__ = __str__ = lambda self: (
        "Engines(" + ", ".join(f"{k}={self.__dict__[k]}" for k in self.__dict__) + ")"
    )
...
class WingLoads:
    ...
    __repr__ = __str__ = lambda self: (
        "Wingloads(" + ", ".join(f"{k}={self.__dict__[k]}" for k in self.__dict__) + ")"
    )
    ...
    def equilibriumCruise(self, dragDistr, liftDistr, macDistr, wingWeight):
        ...
        Thrusts = [
            PointLoad(
                [-self.engines.Tcruise, -self.engines.w, 0],
                [-0.45 * self.wing(p).b / self.frac, 0, p],
            )
            for p in self.engines.pos
        ]  # Redefine x, y
        Drag = RunningLoad(
            [dragDistr[1], [0] * len(dragDistr[1])], dragDistr[0], axis=2, poa=(acp, 0)
        )
        Lift = RunningLoad(
            [[0] * len(liftDistr[1]), liftDistr[1]], liftDistr[0], axis=2, poa=(acp, 0)
        )
        Mac = DistributedMoment(
            values=[[0, 0, mom] for mom in macDistr[1]], positions=macDistr[0]
        )
        weights = RunningLoad(
            values=[[0] * 2, [-wingWeight / self.span] * 2],
            positions=[0, self.span / 2],
            axis=2,
        )
        eqn = EquilibriumEquation(
            kloads=[Lift, weights, Drag, Mac] + Thrusts,
            ukloads=[
                PointLoad([1, 0, 0], [0, 0, 0]),
                PointLoad([0, 1, 0], [0, 0, 0]),
                PointLoad([0, 0, 1], [0, 0, 0]),
                Moment([1, 0, 0]),
                Moment([0, 1, 0]),
                Moment([0, 0, 1]),
            ],
        ...
        eqn.SetupEquation()
        self.RFx, self.RFy, self.RFz, self.RMx, self.RMy, self.RMz = solved = (
            eqn.SolveEquation()
        )
        return solved
    ...
    def equilibriumVTO(self, wingWeight, ground=False):
        Thrusts = [
            PointLoad(
                [(-self.engines.Thover if not ground else 0) + self.engines.w, 0, 0],
                [-0.45 * self.wing(p).b / self.frac, 0, p],
            )
            for p in self.engines.pos
        ]  # Redefine x, y
        weights = RunningLoad(
            values=[[wingWeight / self.span] * 2, [0] * 2],
            positions=[0, self.span / 2],
            axis=2,
        )
        eqtn = EquilibriumEquation(
            kloads=[weights] + Thrusts,
            ukloads=[
                PointLoad([1, 0, 0], [0, 0, 0]),
                Moment([0, 1, 0]),
                Moment([0, 0, 1]),
    ...
    def internalLoads(self, dragDistr, liftDistr, macDistr, wingWeight):
        ...
        drag = StepFunction(
            [[dragcoef[i], 0, i] for i in range(len(dragcoef))] + [[interd, 0, 0]]
        )
        lift = StepFunction(
            [[liftcoef[i], 0, i] for i in range(len(liftcoef))] + [[interl, 0, 0]]
        )
        Mac = StepFunction(
            [[momcoef[i], 0, i] for i in range(len(momcoef))] + [[interm, 0, 0]]
        )
        ...
        MThrust = StepFunction(
            [
                [0.45 * self.wing(p).b * self.engines.w / self.frac, p, 0]
                for p in self.engines.pos
            ]
        )
    ...
    def internalLoadsVTO(self, wingWeight, ground=False):
        ...
        Thrusts = StepFunction(
            [
                [(-self.engines.Thover if not ground else 0) + self.engines.w, p, 0]
                for p in self.engines.pos
            ]
        )  # Fx
    ...
    def stressesCruise(self, point=0):
        ...
        # q, tau, o
        coordinates = (
            [[x, root.h / 2] for x in np.linspace(-root.b / 2, root.b / 2, 1000)]
            + [[root.b / 2, y] for y in np.linspace(root.h / 2, -root.h / 2, 1000)]
            + [[x, -root.h / 2] for x in np.linspace(-root.b / 2, root.b / 2, 1000)]
            + [[-root.b / 2, y] for y in np.linspace(root.h / 2, -root.h / 2, 1000)]
        )
        ...
        tau = np.array(
            [
                root.tau(ix, iy, self.Vx(point), self.Vy(point), self.T(point))
    ...
    def stressesVTO(self, point=0):
        ...
        tau = np.array(
            [root.tau(ix, iy, Vx=self.ViVx(point), Vy=0, T=0) for ix, iy in coordinates]
        )
        return np.array(coordinates), sigma, tau, np.sqrt(3 * tau**2 + sigma**2)
...
class Lug:
    def __init__(self, a, c, d, t, mat=7075):
        ...
        self.b, self.m = (
            ...
            if mat == 7075
            else (5.929411764705884, -0.011764705882352944)
        )
        self.density = 2810
        self.mat = mat
    ...
    mass = lambda self, L: (
        ...
        * self.density
        * (
            np.pi * ((self.a + self.d / 2) ** 2 - self.d**2 / 4)
            + L * (2 * self.c + self.d)
        )
    )

    Fatigue = lambda self, F, alpha=0: self.SN(
        self.Kt() * self.Ka(alpha) * self.stress(F)
    )

    SN = lambda self, S: 10 ** (self.b + S * self.m)

    __repr__ = __str__ = lambda self: (
        "Lug(" + ", ".join(f"{k}={self.__dict__[k]}" for k in self.__dict__) + ")"
    )
...
class Fatigue:
    def __init__(self, Sground, Stakeoff, Scruise, airTime, turnOver, takeOffTime, mat):
        self.Sg, self.Sto, self.Scr, self.tAir, self.tTO = (
            Sground,
            Stakeoff,
            Scruise,
            airTime,
    ...
    def determineCycle(self):
        ...
        gag, tg = [], self.tot / 2
        for t in self.ts:
            acoustic = 0.05 * np.sum(
                np.sin(
                    2 * np.pi * t / np.arange(0.001 / 3600, 0.01 / 3600, 0.001 / 3600)
                )
            )  # noise in the given frequency range
            turbulence = 0.1 * np.sum(
                np.sin(2 * np.pi * t / np.arange(0.1 / 3600, 10 / 3600, 0.1 / 3600))
            )  # same here
            taxi = 0.1 * np.sum(
                np.sin(2 * np.pi * t / np.arange(0.05 / 3600, 1 / 3600, 0.05 / 3600))
            )  # and here, all frequencies taken from the textbook - Schijve Ch 9
            if t < tg or t > self.tAir + tg:
                gag.append(
                    ...
                    + (
                        taxi
                        if tg > t > 0.5 * tg
                        or self.tAir + tg < t < self.tAir + 1.5 * tg
                        else 0
                    )
                )
                ...
                gag.append(
                    self.Scr
                    + acoustic
                    + (turbulence if tg + 1 > t or t > self.tAir + tg - 1 else 0)
                )
            else:
    ...
    def getCycles(self):
        df = pd.DataFrame(
            data=list(rainflow.extract_cycles(self.cyc)),
