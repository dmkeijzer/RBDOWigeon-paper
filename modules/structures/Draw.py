...
def PlotContour(xp, y, z, xtit, ytit, title, x):
    fig = go.Figure(
        data=go.Heatmap(
            x=xp, y=y, z=z, connectgaps=False, colorscale="Picnic", zsmooth="best"
        ),
        layout=go.Layout(
...
def NormalStressPlot(oxx, x, Ch, Ca, shape, tsk, **others):
    ...
    o_zy = [
        [None if abs(y_i) > shape(z_i) else oxx(z_i, y_i)(x) * 1e-6 for z_i in zi]
        for y_i in yi
    ]
    return PlotContour(
        zi,
...
def ShearStressPlot(oxx, x, Ch, Ca, shape, tsk, tsp, **others):
    ...
    o_zy = [
        [
            None
            if abs(y_i) > shape(z_i)
            or (abs(y_i) < shape(z_i) - tsk and not -tsp < z_i < tsp)
            else oxx(z_i, y_i)(x) * 1e-6
            for z_i in zi
        ]
        for y_i in yi
    ]
...
def InternalLoading(x0, x1, **Loads):
    ...
    fig = make_subplots(
        rows=len(list(Loads.keys())), cols=1, shared_xaxes=True, vertical_spacing=0.02
    )
...
def Deflections(x0, x1, pi, **defs):
    titles = [
        k + f" [{'millimeters' if k[0].lower() in ['w', 'v'] else 'degrees'}]"
        for k in defs
    ]
    fig = make_subplots(
        rows=len(list(defs.keys())), cols=1, shared_xaxes=True, vertical_spacing=0.01
    )
    xs = linspace(x0, x1, 5000)
    for i, (li, load) in enumerate(zip(defs.keys(), defs.values())):
        fig.append_trace(
            go.Scatter(
                x=xs,
                y=[
                    load(xi) * (180 / pi if "degrees" in titles[i] else 1e3)
                    for xi in xs
                ],
                name=li,
            ),
    ...
    fig.update_xaxes(
        title_text="Spanwise Position [meters]", row=len(defs.keys()), col=1
    )
    fig.update_layout(title="Deflection Diagram")
