...
class Structure:
    def __init__(self, **inputs):
        ...
        base, height = (
            0.75 - 0.15,
            0.11571117509557907 + 0.03145738125495376,
        )  # x/c, y/c
        self.pos = np.linspace(0, self.span1 / 2, len(self.CL))
        lift_area = RunningLoad(
            [np.array(self.CL), [0] * len(self.CL)], positions=self.pos, axis=2
        ).force()[0]
        drag_area = RunningLoad(
            [np.array(self.CD), [0] * len(self.CD)], positions=self.pos, axis=2
        ).force()[0]
        self.n_ult = self.nmax * 1.5
        self.liftd = (
            np.array(self.CL) / lift_area * self.n_ult * self.mtom / 4 * 9.80665
        )
        self.dragd = np.array(self.CD) / drag_area * (self.Tcruise * 12) / 4
        self.span = self.span2 if self.w_back else self.span1
        self.hatGeom = dict(
            bflange1=self.rootchord / (self.ntofit * 3),
        ...
        self.normalBox = WingBox(
            self.thicknessOfSkin, self.thicknessOfSpar, base, height
        )
        self.normalBox.StrPlacement(
            self.nStrT, self.nStrB, stringerGeometry=self.hatGeom, stringerType="Hat"
        )
        print(self.normalBox.str[0])
        self.wing = WingStructure(self.span, self.taper, self.rootchord, self.normalBox)
        self.enginePlacement = list(
            np.linspace(0.3 + self.wf / 2, self.span1 / 2, int(len(self.pos_prop) / 4))
        )
    ...
    def compute_stresses(
        self, nStrT, nStrB, thicknessOfSkin, thicknessOfSpar, point=0, **kwargs
    ):
        args = dict(
            span=self.span,
            taper=self.taper,
            ...
            xac=self.xAC,
            engines=Engines(
                self.Thover, self.Tcruise, self.enginePlacement, self.engineMass
            ),
            frac=0.6,
        )
        ...
        wing = Wing(
            ...
            self.AR1,
            self.AR2,
            [self.pos_frontwing, self.pos_backwing],
        )
        #         print(wing.mass)
        ...
        w = Weight(
            ...
            cargo_pos=self.cargo_pos,
            battery_m=self.battery_m,
            battery_pos=self.battery_pos,
            p_pax=self.p_pax,
        )

        lift, wgt = self.loads.internalLoads(
            [self.pos, self.dragd],
            [self.pos, self.liftd],
            [self.pos, [self.Mac / self.span] * len(self.pos)],
            wingWeight,
        )

        if self.cruise:
            reactionsCruise = self.loads.equilibriumCruise(
                [self.pos, self.dragd],
                [self.pos, self.liftd],
                [self.pos, [self.Mac / self.span] * len(self.pos)],
                self.wingWeight,
            ...
            (self.omin, self.omax), (self.taumin, self.taumax) = (
                WingLoads.extreme(coords, o_cr),
                WingLoads.extreme(coords, tau_cr),
            )
            ...
            (self.omin, self.omax), (self.taumin, self.taumax) = (
                WingLoads.extreme(coords, o_VTO),
                WingLoads.extreme(coords, tau_VTO),
            )

        if self.omin[1] > 0:
            raise StructuralError(
                "Positive Compression Stress Encountered: " + str(omin[1])
            )
    ...
    def compute_fatigue(self, matsk):
        ...
        pos, liftd, dragd, Mac, span, wingWeight = [
            self[k] for k in "pos, liftd, dragd, Mac, span, wingWeight".split(", ")
        ]

        fatigue_reactionsCruise = self.loads.equilibriumCruise(
            [pos, dragd], [pos, liftd], [pos, [Mac / span] * len(pos)], wingWeight
        )
        fatigue_lift, fatigue_wgt = self.loads.internalLoads(
            [pos, dragd], [pos, liftd], [pos, [Mac / span] * len(pos)], wingWeight
        )
        ...
        fatigue_VxVTOgr, fatigue_MyVTOgr = self.loads.internalLoadsVTO(
            wingWeight, ground=True
        )
        ...
        err = abs(
            (self.cycles - matsk.BasquinLaw(abs(oVTOfgrmd - ocrfmd))) / self.cycles
        )
    ...
    def compute_buckling(self, stringerMat, skinMat, point=0):
        ...
        self.critbuckling = root.Bstress(
            EofStringers, vOfStringers, yieldOfStringers, EofSkin, vOfSkin
        )
    ...
    def optimize(self, point=0):
        ...
        nStrT, nStrB, thicknessOfSkin, thicknessOfSpar = [
            self[k]
            for k in "nStrT, nStrB, thicknessOfSkin, thicknessOfSpar".split(", ")
        ]

        while True:
            (omin, omax), (taumin, taumax), (Ymin, Ymax) = self.compute_stresses(
                nStrT, nStrB, thicknessOfSkin, thicknessOfSpar
            )
            root = self.loads.wing(point)
            ...
            yieldMargin, bucklingmargin = (
                self.matsk.oy / Ymax[1],
                abs(critbuckling / omin[1]),
            )
            ...
            if omin[1] <= -critbuckling or Ymax[1] >= self.matsk.oy:
                ...
                if bucklingmargin > yieldMargin:
                    if (
                        abs(Ymax[0][0] - root.b / 2) <= 1.5 * root.tsp
                        or abs(Ymax[0][0] + root.b / 2) <= 1.5 * root.tsp
                    ):
                        thicknessOfSpar += 0.0001
                    elif (
                        abs(Ymax[0][1] - root.h / 2) <= root.tsk
                        or abs(Ymax[0][1] + root.h / 2) <= root.tsk
                    ):
                ...
                if self.cycles < 15 * 365 * 4:
                    ...
                    cracklength, ncycs = self.fatigue.CrackGrowth(
                        1.2 * 0.375 / 1000, root.tsk, round(self.cycles)
                    )
                    print(
                        f"Fatigue Life: {round(self.cycles / 1e3) * 1000} cycles, Damage tolerance: {cracklength * 1e3} critical crack length [mm], {ncycs} cycles"
                    )
                    break
            else:
                print("Success\n")
        ...
        self.nStrT, self.nStrB, self.thicknessOfSkin, self.thicknessOfSpar = (
            nStrT,
            nStrB,
            thicknessOfSkin,
            thicknessOfSpar,
    ...
    plotNVMcruise = lambda self: InternalLoading(
        0,
        self.span / 2,
        Vx=self.loads.Vx,
        Vy=self.loads.Vy,
    ...
    plotNVMVTOL = lambda self: InternalLoading(
        0, self.span / 2, Vx=self.loads.ViVx, My=self.loads.ViMy
    )

    plotFatigue = lambda self: DrawFatigue(self.tfat, self.fatcyc)

    process_dist = staticmethod(
        lambda dist: [float(c.strip("\n")) for c in dist.split(" ") if c.strip("\n")][
            ::-1
        ]
    )
