...
class Material:
    def __init__(
        ...
        **others,
        ...
        self.E, self.rho, self.oy, self.oult, self.v = (
            E,
            density,
            o_yield,
            o_ult,
    ...
    __repr__ = __str__ = lambda self: (
        (self.name if self.name else "Material")
        + f"(E={self.E}, ρ={self.rho}, σy={self.oy}, σult={self.oult})"
    )
    ...
    ParisFatigueda = lambda self, a, w, Smax, Smin, n: (
        (0.5 + 0.4 * abs(Smin / Smax))
        * n
        * self.C
        * (self.beta(a / w) * abs(Smax - Smin) * (pi * a) ** 0.5) ** self.m
    )
