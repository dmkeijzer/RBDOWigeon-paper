...
class PointLoad:
    ...
    __repr__ = __str__ = lambda self: (
        f"Pointload(Force={str(self.f)}, Point={str(self.p)})"
    )
...
class Moment:
    ...
    __mul__, __add__ = (
        lambda self, other: Moment(self.m * other),
        lambda self, other: Moment(self.m + other.m),
    )
...
class RunningLoad:
    ...
    def force(self):
        forces = [
            quad(lambda x: np.interp(x, self.p, self.v[i, :]), self.p[0], self.p[-1])[0]
            for i in range(2)
        ]
        f = list(range(3))
    ...
    def moment(self):
        moi = [
            quad(
                lambda x: x * np.interp(x, self.p, self.v[i, :]), self.p[0], self.p[-1]
            )[0]
            for i in range(2)
        ]
        load = [
...
class DistributedMoment:
    ...
    moment = lambda self: np.array(
        [
            quad(
                lambda x: np.interp(x, self.p, self.v[i, :]),
                self.p[0],
                self.p[-1],
            )[0]
            for i in range(3)
...
class EquilibriumEquation:
    ...
    def SetupEquation(self):
        b = np.concatenate(
            [-sum([k.force() for k in self.k]), -sum([l.moment() for l in self.k])]
        ).reshape(-1, 1)
    ...
    SolveEquation = lambda self: (
        np.linalg.inv(self.A) @ self.b if len(self.A) and len(self.b) else None
    )
