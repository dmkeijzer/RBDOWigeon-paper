...
class Weight:
    def __init__(
        ...
        self.wweight = (
            np.sum(wing.get_weight()) if wing.config != 3 else wing.get_weight()[0]
        )
        ...
        self.moment_w = (
            np.sum(np.array(wing.get_moment()))
            if type(wing.get_moment()) is tuple
            else wing.get_moment()
        )
    ...
    def print_weight_fractions(self):
        ...
        if type(self.wing.get_weight()) is tuple:
            d["Front wing"] = [
                self.wing.get_weight()[0],
                self.wing.get_weight()[0] / self.oem,
                self.wing.get_weight()[0] / self.mtom,
            ]
            d["Back wing"] = [
                self.wing.get_weight()[1],
                self.wing.get_weight()[1] / self.oem,
                self.wing.get_weight()[1] / self.mtom,
            ]
        else:
            d["Wing"] = [
                self.wing.get_weight(),
                self.wing.get_weight() / self.oem,
                self.wing.get_weight() / self.mtom,
            ]
        d["Fuselage"] = [
            self.fweight,
            self.fweight / self.oem,
            self.fweight / self.mtom,
        ]
        d["Landing gear"] = [
            self.lweight,
            self.lweight / self.oem,
            self.lweight / self.mtom,
        ]
        d["Propulsion"] = [
            self.pweight,
            self.pweight / self.oem,
            self.pweight / self.mtom,
        ]
        ...
        for key in d:
            d[key] = {
                k: list(i) if isinstance(i, np.ndarray) else i
                for k, i in zip(["mass", "fracOEM", "fracEM"], d[key])
            }
        return d
