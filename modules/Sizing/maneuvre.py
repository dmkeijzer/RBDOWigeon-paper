...
def plotmaneuvrenv(WoS, Vc, CLmax):
    ...
    ax = sns.lineplot(
        x=x, y=[maneuvrenv(V, Vs, WoS, CLmax, True) for V in x], color="red"
    )
    sns.lineplot(
        x=x,
        y=[maneuvrenv(V, Vs, WoS, CLmax, False) for V in x],
        color="red",
        label="Maneuvre Envelope",
    )
    ax.set(xlabel="V [m/s]", ylabel="n [-]")
    plt.plot(
        [VD, VD],
        [maneuvrenv(VD, Vs, WoS, CLmax, False), maneuvrenv(VD, Vs, WoS, CLmax, True)],
        color="red",
    )
...
def posgustload(V, Vs, us, ns, CLalpha, WoS):
    ...
    return (
        ...
        if 0 <= V <= Vb
            ...
            interpolate(V, Vb, Vc, nb, nc)
            if Vb < V <= Vc
            else interpolate(V, Vc, VD, nc, nd)
        )
    )


neggustload = lambda V, Vs, us, ns, CLalpha, WoS: (
    2 - posgustload(V, Vs, us, ns, CLalpha, WoS)
)
...
def plotgustenv(Vb, Vc, CLalpha, WoS):
    ...
    x = np.linspace(0, VD, 100)
    ax = sns.lineplot(
        x=x, y=[posgustload(V, Vs, us, ns, CLalpha, WoS) for V in x], color="blue"
    )
    ax.set(xlabel="V [m/s]", ylabel="n [-]")
    sns.lineplot(
        x=x,
        y=[neggustload(V, Vs, us, ns, CLalpha, WoS) for V in x],
        color="blue",
        label="Gust Load Envelope",
    )
    plt.plot(
        [VD, VD],
        [
            neggustload(VD, Vs, us, ns, CLalpha, WoS),
            posgustload(VD, Vs, us, ns, CLalpha, WoS),
        ],
        color="blue",
    )
