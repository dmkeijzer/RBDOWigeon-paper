...
from modules.plotting.MCS_metric_deterministic_reader import (
    determinisic_energy_pdf_cdf_plot,
)
...
def plot_pdf_cdf():
    ...
    dump_path = os.path.join(
        os.path.join(
            dir_path, "plots", "compare_baseline_" + label_deter + "_mcs" + label_mcs
        )
    )

    npz_reader = npz_tool(save_bool=False)
    data_csv = pd.read_csv(
        os.path.join(dir_path, "Deterministic" + label_deter + "_hist.csv")
    )
    conv_lst = np.array(np.round(data_csv["converged"], 0), dtype=bool)

    x_deter, pdf_deter, cdf_deter = determinisic_energy_pdf_cdf_plot(
        False, return_data=True
    )
    ...
    plt.vlines(
        [data_csv["Energy"][conv_lst].to_numpy()[-1] / 3.6e6],
        0,
        pdf_deter[
            np.argmin(np.abs(x_deter - data_csv["Energy"][conv_lst].to_numpy()[-1]))
        ],
        color="red",
        alpha=0.3,
    )
    plt.fill_between(
        x_deter / 3.6e6,
        pdf_deter,
        np.zeros(np.size(x_deter)),
        where=(x_deter > data_csv["Energy"][conv_lst].to_numpy()[-1]),
        color="red",
        alpha=0.3,
    )
    ...
    plt.vlines(
        [dist.ppf(0.9) / 3.6e6],
        0,
        pdf_mcs[np.argmin(np.abs(x_mcs - dist.ppf(0.9)))],
        color="red",
        alpha=0.3,
    )
    plt.fill_between(
        x_mcs / 3.6e6,
        pdf_mcs,
        np.zeros(np.size(x_mcs)),
        where=(x_mcs > dist.ppf(0.9)),
        color="red",
        alpha=0.3,
    )
    ...
    plt.vlines(
        [dist.ppf(0.9) / 3.6e6],
        0,
        cdf_mcs[np.argmin(np.abs(x_mcs - dist.ppf(0.9)))],
        color="red",
        alpha=0.3,
    )
    plt.fill_between(
...
def compare_weights():
    ...
    deter_path = os.path.join(
        deter_run_path, "Deterministic" + label_deter + "_hist.csv"
    )
    ...
    vtail_robust = data_robust.df["Vertical_tail_mass"][
        data_robust.conv_lst
    ].to_numpy()[-1]

    # Power train  mass
    powertrain_deter = data_deter["Vertical_tail_mass"].to_numpy()[-1] * 12
    powertrain_robust = (
        data_robust.df["Vertical_tail_mass"][data_robust.conv_lst].to_numpy()[-1] * 12
    )
    ...
    comp_robust = [
        wing_robust.wweight1,
        wing_robust.wweight2,
        vtail_robust,
        m_bat_robust,
        fuse_robust.mass,
        powertrain_robust,
    ...
    axes[0].pie(
        comp_deter,
        labels=str_lst,
        explode=np.ones(len(comp_robust)) * 0.0,
        autopct="%1.1f%%",
        startangle=90,
    ...
    fig.tight_layout()
    plt.savefig(
        os.path.join(dump_path, "weight_pie_chart_comparison" + label_mcs + ".pdf"),
        bbox_inches="tight",
    )
    # plt.show()
