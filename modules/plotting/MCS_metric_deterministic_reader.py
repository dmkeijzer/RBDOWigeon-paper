...
def hover_loiter_energy():
    ...
    plt.figure(figsize=(10,10))
    hist, edges, patches = plt.hist(Eloit_hov_rv.data/3.6e6, density= True, lw= 2, rwidth = 0.85, color="lightsteelblue",  edgecolor="k")
    plt.ylim([0,1.1*np.max(hist[1:])])
    plt.title("Loiter hover conf")
    plt.xlabel("Energy [KwH]")
    plt.ylabel("[-]")
    plt.grid(lw=0.8, alpha=0.8)
    plt.savefig(os.path.join(download_path, "deterministic_metric_hover_loiter") + "_barchart" +  ".pdf", bbox_inches= "tight")
