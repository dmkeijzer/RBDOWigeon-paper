...
def plot_design_params(save_bool):
    ...
    if save_bool:
        plt.savefig(
            os.path.join(dump_path_plots, label) + "_DesignParam_" + ".pdf",
            bbox_inches="tight",
        )
    else:
        plt.show()
...
def plot_energy_phases(save_bool):
    ...
    Eloit = data_csv["E_loiter"][
        conv_lst
    ].to_numpy()  # mission independent all samples have the same value
    ...
    if save_bool:
        plt.savefig(
            os.path.join(dump_path_plots, label) + "_EnergyPhases_" + ".pdf",
            bbox_inches="tight",
        )
    else:
        plt.show()
...
def plot_pie_chart_energy(save_bool):
    ...
    Eloit_cr = data_csv["E_loiter"].to_numpy()[
        -1
    ...
    for i, radius, annotation, dx, dy in zip(
        ...
        angle = (
            90
            + sum(energy_lst[:i]) / sum(energy_lst) * 360
            + (energy_lst[i]) / (sum(energy_lst)) * 360 * 1 / 2
        )
    ...
    if save_bool:
        plt.savefig(
            os.path.join(dump_path_plots, label) + "_PieChart_" + ".pdf",
            bbox_inches="tight",
        )
    else:
        plt.show()
