...
NASA_LANGLEY = [
    ...
    0.17,
    0.65,
]  # Lift slope [1/rad], CL_max, C_m cruise, Cd_min, CL for Cdmin, t/c, xcm
EPPLER335 = [
    6.245,
    1.61330,
    0.0489,
...
Wing_planform_params_double = wing_planform_double(
    AR, S1, sweepc41, taper1, S2, sweepc42, taper2
)
...
Cl_des_conv = (
    C_L_des
    / (np.cos(sweep_atx(0, Wing_planform_params_single[1], b, taper, sweepc4))) ** 2
)
Cl_des_box = (
    C_L_des
    / (np.cos(sweep_atx(0, Wing_planform_params_double[0][1], b, taper, sweepc4))) ** 2
)
Cl_des_tan = (
    C_L_des
    / (np.cos(sweep_atx(0, Wing_planform_params_double[0][1], b, taper, sweepc4))) ** 2
...
Re_Number = (
    Re(rho, Vcruise, Wing_planform_params_single[3], mu),
    Re(rho, Vcruise, Wing_planform_params_double[0][3], mu),
    Re(rho, Vcruise, Wing_planform_params_double[1][3], mu),
)
print("Reynolds number", Re_Number)
# Wing performance
sweepc2_single = sweep_atx(
    0.5, Wing_planform_params_single[1], Wing_planform_params_single[0], taper, sweepc4
)
sweepc2_double = sweep_atx(
    0.5,
    Wing_planform_params_double[0][1],
    Wing_planform_params_double[0][0],
    taper,
    sweepc4,
)

Clda_conv = liftslope(
    "normal", AR, sweepc2_single, Mach(Vcruise, a), EPPLER335[0], s1, s2, deda
)  # 2pi airfoil slope assumed as placeholder
Clda_double = liftslope(
    "double", AR, sweepc2_double, Mach(Vcruise, a), NASA_LANGLEY[0], s1, s2, deda
)  # includes in order: total clda, clda wing1, clda wing 2

C_L_max_conv = 0.9 * EPPLER335[1]  # From ADSEE-II L2
C_L_max_double = (
    s1 * 0.9 * NASA_LANGLEY[1] + s2 * 0.9 * 1.930
)  # Due to downwash Clmax for second wing is lower
...
Abase = 0.04
sweep_xcm_single = sweep_atx(
    EPPLER335[6],
    Wing_planform_params_single[1],
    Wing_planform_params_single[0],
    taper,
    sweepc4,
)
sweep_xcm_double = sweep_atx(
    NASA_LANGLEY[6],
    Wing_planform_params_double[0][1],
    Wing_planform_params_single[0],
    taper,
