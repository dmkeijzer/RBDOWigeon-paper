...
def winglet_factor(
    h_wl, b, k_wl
):  # https://www.fzt.haw-hamburg.de/pers/Scholz/Aero/AERO_PUB_Winglets_IntrinsicEfficiency_CEAS2017.pdf
    """_summary_
...
class wing_design:
    ...
    def taper_opt(self):
        return 0.45 * np.exp(-0.036 * self.sweepc41), 0.45 * np.exp(
            -0.036 * self.sweepc42
        )  # Eq. 7.4 Conceptual Design of a Medium Range Box Wing Aircraft
    ...
    def wing_planform_double(self):
        ...
        c_MAC1 = (
            (2 / 3) * c_r1 * ((1 + self.taper1 + self.taper1**2) / (1 + self.taper1))
        )
        y_MAC1 = (b1 / 6) * ((1 + 2 * self.taper1) / (1 + self.taper1))
        tan_sweep_LE1 = 0.25 * (2 * c_r1 / b1) * (1 - self.taper1) + np.tan(
            self.sweepc41
        )

        X_LEMAC1 = y_MAC1 * tan_sweep_LE1
        AReff1 = self.AR1 * winglet_factor(
            self.h_wl1, b1, self.k_wl
        )  # +winglet_dAR(self.AR1, self.h_wl1, b1)
        ...
        c_MAC2 = (
            (2 / 3) * c_r2 * ((1 + self.taper2 + self.taper2**2) / (1 + self.taper2))
        )
        y_MAC2 = (b2 / 6) * ((1 + 2 * self.taper2) / (1 + self.taper2))
        tan_sweep_LE2 = 0.25 * (2 * c_r2 / b2) * (1 - self.taper2) + np.tan(
            self.sweepc42
        )

        X_LEMAC2 = y_MAC2 * tan_sweep_LE2
        AReff2 = self.AR2 * winglet_factor(
            self.h_wl2, b2, self.k_wl
        )  # + winglet_dAR(self.AR2, self.h_wl2, b2)
    ...
    def liftslope(self, deda):
        ...
        slope1 = self.Clalpha * (
            ...
            / (
                2
                + np.sqrt(
                    4 + ((self.AR_i * beta / 0.95) ** 2) * ((1 + SW**2) / (beta**2))
                )
            )
        )
        self.AR_2 = self.AR2 + winglet_dAR(self.AR2, self.h_wl2, wg[0][0])
        slope2_b = self.Clalpha * (
            self.AR_2
            / (
                2
                + np.sqrt(
                    4 + ((self.AR_2 * beta / 0.95) ** 2) * ((1 + SW**2) / (beta**2))
                )
            )
        )
        ...
        # print("de_da",deda)
        wfi = (
            1 + 0.025 * (self.w / wg[0][0]) - 0.25 * (self.w / wg[0][0])
        )  # wing fuselage interaction factor: effect of fuselage diameter on aerodynamic characteristics for straightwing at low and high aspect ratio
    ...
    def CLmax_s(self, de_da):
        ...
        alpha_s2 = (
            np.round(((self.a_saf - self.i1 - self.a_0L) * (1 - deda) + self.a_0L) * 4)
            / 4
        )
    ...
    def post_stall_lift_drag(self, tc, CDs_W, CDs_f, Afus, de_da):
        ...
        A2 = (CLs - 2 * A1 * np.sin(a_s) * np.cos(a_s)) * (
            np.sin(a_s) / (np.cos(a_s) ** 2)
        )
        ...
        CL_ps = A1 * np.sin(2 * alpha_ps) + A2 * (
            (np.cos(alpha_ps) ** 2) / np.sin(alpha_ps)
        )
        ...
        CDmax_f = (
            1.18 * Afus / self.S
        )  # https://sv.20file.org/up1/916_0.pdf Drag coefficient of a cylinder
    ...
    def deltaV(self, T, V_inf, rho, D, ne1, ne2):
        ...
        return V_inf * (
            np.sqrt(
                1
                + (self.C_T(ne1, ne2, T, V_inf, rho) * self.S)
                / ((ne1 + ne2) * (np.pi / 4) * self.D**2)
            )
            - 1
        )
    ...
    def Deff(self, T, V_inf, rho, D, ne1, ne2):
        return D * np.sqrt(
            (V_inf + self.deltaV(T, V_inf, rho, D, ne1, ne2) * 0.5)
            / (V_inf + self.deltaV(T, V_inf, rho, D, ne1, ne2))
        )

    def a_ss(self, T, V_inf, rho, D, alpha_wp, ne1, ne2):

        return (180 / np.pi) * np.arctan2(
            V_inf * np.sin(alpha_wp * np.pi / 180),
            (
                V_inf * np.cos(alpha_wp * np.pi / 180)
                + self.deltaV(T, V_inf, rho, D, ne1, ne2) * 0.5
            ),
        ) - self.a_0L
    ...
    def Aseff(self, n_e1, n_e2, T, V_inf, rho, D):
        ...
        ASeff1 = AS1 + (self.AR1 - AS1) * (
            V_inf / (self.deltaV(T, V_inf, rho, D, n_e1, n_e2) + V_inf)
        ) ** (self.AR1 - AS1)
        ASeff2 = AS2 + (self.AR2 - AS2) * (
            V_inf / (self.deltaV(T, V_inf, rho, D, n_e1, n_e2) + V_inf)
        ) ** (self.AR2 - AS2)
        return ASeff1, ASeff2

    def CL_W_S(
        self, T, V_inf, rho, D, n_e1, n_e2, tc, CDs_W, CDs_f, Afus, alpha_wp, de_da
    ):
        self.b1 = np.sqrt(self.AR1 * self.S1)
        self.b2 = np.sqrt(self.AR2 * self.S2)
        SW = np.tan(self.sweep_atx(0.5))
        ...
        slope1p = self.Clalpha * (
            ARS1
            / (2 + np.sqrt(4 + ((ARS1 * beta / 0.95) ** 2) * ((1 + SW**2) / (beta**2))))
        )
        slope2p = (
            self.Clalpha
            * (
                ...
                / (
                    2
                    + np.sqrt(
                        4 + ((ARS2 * beta / 0.95) ** 2) * ((1 + SW**2) / (beta**2))
                    )
                )
            )
            * (1 - de_da)
        ...
        pt1 = (
            2
            * self.CLa(tc, CDs_W, CDs_f, Afus, alpha_wp, de_da)[1]
            / (np.pi * self.AR1)
        )
        pt2 = (
            2
            * self.CLa(tc, CDs_W, CDs_f, Afus, alpha_wp, de_da)[2]
            / (np.pi * self.AR2)
        )
        pt3 = (
            2
            * slope1p[0]
            * np.sin(self.a_ss(T, V_inf, rho, D, alpha_wp, n_e1, n_e2) * np.pi / 180)
            / (np.pi * ARS1)
        )  # 2*self.CLa(tc, CDs_W, CDs_f, Afus, self.a_ss(T, V_inf, rho, D , alpha_wp, n_e1, n_e2), de_da)[1]*np.sin(self.a_ss(T, V_inf, rho, D , alpha_wp, n_e1, n_e2)*np.pi/180)/(np.pi*ARS1)    #self.a_ss(T, V_inf, rho, D , alpha_wp)
        pt4 = (
            2
            * slope2p[1]
            * np.sin(self.a_ss(T, V_inf, rho, D, alpha_wp, n_e1, n_e2) * np.pi / 180)
        ...
        self.CLW1 = (
            ...
            * (
                (np.pi / 4) * self.b1**2
                - n_e1 * (np.pi / 4) * (self.Deff(T, V_inf, rho, D, n_e1, n_e2)) ** 2
            )
            * pt1
        )
        self.CLW2 = (
            ...
            * (
                (np.pi / 4) * self.b2**2
                - n_e2 * (np.pi / 4) * (self.Deff(T, V_inf, rho, D, n_e1, n_e2)) ** 2
            )
            * pt2
        )
        self.CLS1 = (
            n_e1
            * (np.pi * (self.Deff(T, V_inf, rho, D, n_e1, n_e2) ** 2) / (2 * self.S1))
            * ((V_inf + self.deltaV(T, V_inf, rho, D, n_e1, n_e2)) ** 2 / V_inf**2)
            * pt3
        )
        self.CLS2 = (
            n_e2
            * (np.pi * (self.Deff(T, V_inf, rho, D, n_e1, n_e2) ** 2) / (2 * self.S2))
            * ((V_inf + self.deltaV(T, V_inf, rho, D, n_e1, n_e2)) ** 2 / V_inf**2)
            * pt4
        )
    ...
    def CLa_wprop(
        self, T, V_inf, rho, D, ne1, ne2, tc, CDs_W, CDs_f, Afus, alpha_wp, de_da
    ):
        self.CLmax_s(de_da)

        alpha = np.arange(-5, self.a_s[1] + 1, 0.25)
        CLaw1 = self.CL_W_S(
            T, V_inf, rho, D, ne1, ne2, tc, CDs_W, CDs_f, Afus, alpha, de_da
        )[0] + self.CL_T(T, V_inf, rho, alpha, ne1, ne2)
        CLaw2 = self.CL_W_S(
            T, V_inf, rho, D, ne1, ne2, tc, CDs_W, CDs_f, Afus, alpha, de_da
        )[1] + self.CL_T(T, V_inf, rho, alpha, ne1, ne2)
        CLaw = self.s1 * CLaw1 + self.s2 * CLaw2
        slope1wp = linregress(
            (np.pi / 180) * alpha[0 : int(len(alpha))], CLaw1[0 : int(len(alpha))]
        )[0]
        slope2wp = linregress(
            (np.pi / 180) * alpha[0 : int(len(alpha))], CLaw2[0 : int(len(alpha))]
        )[0] * (1 / (1 - de_da))
