...
def airfoil_stats():
    ...
    Clalpha = (
        np.average(df1["CL"][df1["alpha"] == 8])
        - np.average(df1["CL"][df1["alpha"] == 0])
    ) / 8
    Clalpha1 = (
        np.average(df1["CL"][df1["alpha"] == 5])
        - np.average(df1["CL"][df1["alpha"] == -1])
    ) / 6
...
def Cm_ac(sweep, ARw):
    ...
    CN_lst = np.array(df1["CL"][(df1["alpha"] < 5) & (df1["alpha"] > -3)]) * np.cos(
        alpha * np.pi / 180
    ) + np.array(df1["CD"][(df1["alpha"] < 5) & (df1["alpha"] > -3)]) * np.sin(
        alpha * np.pi / 180
    )
