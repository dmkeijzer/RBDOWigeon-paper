...
def CL_des(rho, V, W, S):
    return (
        W / S / (0.5 * rho * V**2)
    )  # no -ve lift contribution from tail trimming -> no 10% factor
...
def liftslope(type, AR, sweepc2, M, Clda_airfoil, s1, s2, deda):
    b = np.sqrt(1 - M**2)
    SW = np.tan(sweepc2)
    ref_slope = Clda_airfoil * (
        AR / (2 + np.sqrt(4 + ((AR * b / 0.95) ** 2) * ((1 + SW**2) / (b**2))))
    )
    ...
    if type == "normal":
        ...
        ratio = (
            2
            * (2 + np.sqrt((AR**2) * (1 + SW**2) + 4))
            / (2 + np.sqrt((4 * AR**2) * (1 + 0.25 * SW**2) + 4))
        )
