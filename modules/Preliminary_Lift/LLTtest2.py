...
def weissinger_l(wing, al, m, AR, a_w, iw):
    ...
    e = (CL**2) / (np.pi * AR * CDi)
    return y * wing.span / 2.0, cl, ccl, al_i * 180.0 / pi, CL, CDi, e
...
def LLT2wings(
    ...
    y, cl, ccl, al_i, CL, CDi, e = weissinger_l(
        wing, alpha1, 2 * npoints - 1, AR1, np.zeros(43), i1
    )
    ...
    y3, cl3, ccl3, al_i3, CL3, CDi3, e3 = weissinger_l(
        wing2, alpha2, 2 * npoints - 1, AR2, a_w, i2
    )
...
def downwash(
    span1, AR1, root1, tip1, sweep1, alpha1, z_h, x_h, span2, root2, tip2, sweep2, V_cr
):
    npoints = 11
    washout = 0
    wing = Wing(span1, root1, tip1, sweep1, washout)
    y, cl, ccl, al_i, CL, CDi, e = weissinger_l(
        wing, alpha1, 2 * npoints - 1, AR1, np.zeros(2 * npoints + 1), 0
    )
    wing2 = Wing(span2, root2, tip2, sweep2, washout)
    y2 = gety(wing2, 2 * npoints - 1)
    a_w = downwash_fore(np.append(0, ccl[1:] / cl[1:]), y, y2, cl, x_h, z_h, V_cr)
...
def downwash_upwash(
    ...
    de_da = np.average(a_w) * (180 / (np.pi * alpha1))
    y3, cl3, ccl3, al_i3, CL3, CDi3, e3 = weissinger_l(
        wing2, alpha1, 2 * npoints - 1, AR2, a_w, 0
    )
    # print(a_w)
    a_w2 = downwash_fore(
        np.append(0, ccl3[1:] / cl3[1:]), y3, y, cl3, -1 * x_h, -1 * z_h, V_cr
    )
...
def LLT1wing(span1, AR1, root1, tip1, sweep1, alpha1):
    ...
    wing = Wing(span1, root1, tip1, sweep1, washout)
    y, cl, ccl, al_i, CL, CDi, e = weissinger_l(
        wing, alpha1, 2 * npoints - 1, AR1, np.zeros(43), 0
    )
    # return CL, CDi, e,
...
if __name__ == "__main__":
    ...
    y2, cl2, ccl2, al_i2, CL2, CDi2, e2 = LLT2wings(
        8.704,
        4.5,
        1.10695,
        0.49813,
