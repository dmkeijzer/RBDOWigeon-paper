...
def test_randvar():
    ...
    assert np.isclose(ppf_test, 0.0, atol=1e-1)
    assert np.isclose(expect_test, 0.0, atol=1e-1)
    assert np.isclose(std_test, 1.0, atol=1e-1)
...
def test_get_mcs_results(mission_class_mcs):
    ...
    results, sample_history, conver_hist = mcs.get_mcs_results(
        mission_class_mcs, 0.7, chunksize=12, test=True
    )
