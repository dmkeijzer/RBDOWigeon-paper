...
class testPerformance(unittest.TestCase):
    def test_max_speed(self):
        ...
        performance = evtol_performance(
            300,
            60,
            S=S,
            CL_max=1.5,
            mass=2000,
            battery_capacity=500e6,
            EOM=1500,
            loiter_time=30 * 60,
            A_disk=A_disk,
            P_max=P_max,
    ...
    def test_vertical_climb(self):
        ...
        V_fall = -np.sqrt(
            2 * performance.W / (performance.CD_vert * rho * performance.S)
        )

        # Prediction using actual model
        V_mod = performance.vertical_equilibrium(
            300, testing=True, test_thrust=0, m=mass
        )

        self.assertAlmostEqual(V_fall, V_mod, delta=2)

        # Test what happens when thrust equals weight
        RC_hover = performance.vertical_equilibrium(
            300, mass, testing=True, test_thrust=mass * g
        )
    ...
    def test_max_thrust(self):
        ...
        perf = evtol_performance(
            cruising_alt=300,
            cruise_speed=60,
            CL_max=1.5,
            A_disk=10,
            S=14,
            battery_capacity=4e6 * 1.2,
            mass=3000,
            EOM=3000 - 475,
            loiter_time=30 * 60,
            P_max=3000000,
    ...
    def test_range_and_energy(self):
        ...
        miss = mission(
            ...
            t_loiter=30 * 60,
            rotational_rate=5,
            mission_dist=target_range,
        )

        E_tot, t_tot = miss.single_sample_monte_carlo()
        ...
        R, t = perf.range(
            cruising_altitude=h_cr, cruise_speed=v_cr, mass=mass, loiter=True
        )
