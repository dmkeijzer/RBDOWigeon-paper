...
class testAccelerations(unittest.TestCase):
    def test_zero_conditions(self):
        ...
        dist, E, time = standstill.numerical_simulation(
            vx_start=1e-9,
            y_start=0,
            th_start=np.pi / 2,
            y_tgt=0,
    ...
    def test_straight_up(self):
        ...
        dist, E, time = vertical.numerical_simulation(
            vx_start=1e-9,
            y_start=0,
            th_start=np.pi / 2,
            y_tgt=300,
    ...
    def test_forward(self):
        ...
        dist_up, E_up, time_up = cruise.numerical_simulation(
            vx_start=60,
            y_start=300,
            th_start=np.radians(1),
            y_tgt=300 + 1e-9,
            vx_tgt=60,
            plotting=True,
        )

        dist_dn, E_dn, time_dn = cruise.numerical_simulation(
            vx_start=60,
            y_start=300,
            th_start=np.radians(1),
            y_tgt=300 - 1e-9,
