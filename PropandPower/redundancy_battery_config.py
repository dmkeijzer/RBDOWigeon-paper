...
class redundancy_power:
    def __init__(self, V_motor, E_tot, V_cell, C_cell, n_mot, n_bat_mot, per_mot):
        """
        :param V_motor: Voltage required for the electric motors
        :param E_tot: total energy required
        :param P_in_moter: required power for the motor
        :param V_cell: voltage of a single cell
        :param C_cell: capacitance of a single cell
    ...
    def increase_mot(self):
        ...
        perc = np.round(((increase) / self.N_cells_mot_new() * 100), 5)
        return perc

    def increase(self):
        increase = self.N_cells_new() - self.N_cells_tot()
        perc = np.round(((increase) / self.N_cells_tot() * 100), 2)
